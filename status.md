...
* [x] 全选后**打字**同样修复（整篇替换），并带负控：带块语法的字符仍拒绝
...
全选后**打字**是同一个手势带内容的版本，已一并修复：detector 现在也接受
「单个 textblock、纯文本、无 mark」，候选字节就是打出来的文本。

**证明不放松**：非空替换必须重解析后与 PM 即将显示的文档逐节点相等，否则具名拒绝。
