...
- **逐字输入任务列表** — 输入 `- [ ] `、`* [ ] ` 或 `+ [ ] ` 后会立即显示未完成任务框；继续输入正文仍保存为任务 Markdown，而不会变成转义的普通列表文字。输入 `[x]` 同样创建已完成任务。普通源码中的裸 `* [ ]` 保持普通文字列表，冷重开不会被自动改回任务框。
- **空任务项源码优先** — 删除任务项最后一个文字后，编辑器会先将 live checkbox 节点转换为普通列表文本，再保存为 `* [ ]` / `* [x]`；不写入实体、注释或 HorseMD 私有哨兵。该语法在 GFM 中就是普通列表文字，冷重开及其他 Markdown 工具都会得到同一结果，而不会伪装成可跨产品保留的空复选框。
- **有序列表源码统一** — 富文本操作若产生标准 Markdown 无法独立表示的相邻有序列表，会在富文本中合并为一个连续 `1.` 列表；不再为了维持内部边界写入 `1)` 或隐藏注释。作者原本使用的 `1)` 仍按原样保留。
- **空格源码不再依赖私有哨兵** — 段落、引用和列表项末尾新输入的普通 ASCII 空格按字面空格写入 Markdown，不再变成 `&#x20;`；行首需要保留的空格改为标准 `&nbsp;` 加其余字面空格，不再写入零宽私有字符，其他 Markdown/HTML 工具可直接理解。
- **空段落首个 Tab 可安全保存** — 富文本空段落中插入的真实 Tab 现在写为标准 `&#x9;`，避免原始行首 Tab 被 Markdown 解释成缩进代码块，也不会触发保存暂停或使用 HorseMD 专用哨兵。
- **标题边界空白不再阻断保存** — 标题文本的行首/行尾真实 Tab 统一保存为标准 `&#x9;`；普通空格仍分别使用行首 `&nbsp;` 与行尾字面空格，不重新引入 `&#x20;` 或私有哨兵。
- **新增段落空行可保存** — 在两个已有内容块之间按 Enter 产生的空段落现在保存为标准 Markdown 空白行，不再作为编辑器内部 `<br />` 占位被丢弃。
- **空段转列表不再多留空行** — 将这类新建的中间空段直接输入为列表时，列表会接管原有槽位，不会在列表前额外保留一段物理空白。
- **列表中间退出不再锁住源码** — 在有后续条目的有序列表中从空项退出、再按 Enter 时，编辑器只会收敛本次内部拆出的 `.` 列表；嵌套有序列表的同一路径则重建为标准的“空父项 + 缩进子列表”Markdown。后续编号和标题可正常切换源码、保存与重开；不会写入 `<br />` 或合成 `1)`。作者原有的 `<br />` 与 `1)` 仍按源码保留。
- 右键菜单新增「引用/取消引用」（普通模式同样可用）
- **Windows 换行（CRLF）代码块编辑损坏** — 修复在 `\r\n` 文件的代码块里输入时，字符被写到光标之前若干位、`\r\n` 被拆开（磁盘上出现 `;\rX\n}`）、行合并 `Backspace` 只删 `\r` 留下裸 `\n`，以及代码块每次刷新都长出一行永不收敛的幻影空行。位置换算现在按 `\r\n` 对建立双射映射；纯 LF 文档零影响。
- **Mermaid 代码块偶发丢字** — 内置的 Mermaid 语法高亮解析器实现有误（把语言定义工厂函数当成语言定义对象传给 CodeMirror），会在编辑 Mermaid 代码块时抛出异常并让该代码块的 CodeMirror 视图与文档永久失去同步：用户能看到自己在图表源码里打的字，但这些字从未真正写入文档或磁盘。该缺陷同时存在于普通模式和源码权威内核模式，现已修复；不影响其他语言的代码块。

### Internal

- 实验：源码内核支持代码块编辑（LF 围栏）、语言切换与退出；修正段首/连续 Enter 语义；修复内核模式代码块只读失效与 CM 撤销绕过
- 实验：源码内核支持行内格式（加粗/斜体/删除线/行内代码）与右键引用切换；含行内代码的文档不再降级；修复选中格式文本覆写的字节损坏
- 实验：源码内核解除 CRLF/单 `\r` 围栏的收窄（可编辑，行尾原样保留），并拒绝任何会劈开 `\r\n` 对的删除/插入范围
- 实验：源码内核阶段 3 语法域。**治降级**：含行内/块级数学、行内 HTML 片段、`==黄色高亮==` 的文档不再整篇退回普通模式；单个区块证明不了时只把该区块置为只读并单独提示，文档其余部分照常编辑。**扩编辑**：表格单元格文字（`Tab` 导航零字节写入、分隔行与对齐逐字节不变）、黄色高亮工具栏按钮、链接的添加/改网址/移除、图片 `alt`/`src`/`title` 的源码改写命令（目前界面可达的只有空 `![]()` 的 src 输入）。**仍整篇降级**：独占一行的 `$$x$$`、列表项内的块级数学与代码围栏、相邻的根级 `<div>` 兄弟。**仅该区块只读、不拖累整篇文档**：红/蓝高亮 `<mark class>` 所在段落（它是行内 HTML 原子，PM 侧却是多字符标记 run，尺寸不符判该块只读——与黄色高亮不是同一实现，不受黄色高亮解封影响）。**仍拒绝**：块级 HTML、表格行列增删/对齐/建表、含 `<br>` 或 `\|` 的单元格、参差表格、图片 caption/ratio、评阅标记
- 实验：源码内核默认启用前置条件第一批——含行内原子（图片/行内公式/行内 HTML/脚注引用）的段落不再整段拒绝输入，可以绕开原子打字；选中整个原子（含前后文字一起选中）按 `Backspace` 可以整体删除，但部分覆盖原子或删除**带链接的图片**仍拒绝；含 YAML frontmatter 的文档不再整篇降级（frontmatter 块本身仍只读）；内核降级状态现在可见（状态栏内核指示区分「正常 / 部分只读 / 已退回普通模式」）；超过 12 万字符分块加载的文档不再静默退回普通模式，会给出专门的"文档过大"提示；五项安全性能优化中的第一项（内核模式跳过 `markdownUpdated` 序列化通道）已落地，其余四项与"编辑是局部的"这一假设式的增量映射方案均未采纳（方案本身已明确否决，见 `editor-kernel-mode.js` 内的 ADR 注释）——内核默认启用前仍有明显性能差距，详见 `docs/ai-handoff.md` §5.2d。
- 实验：源码内核审计轮四项 Critical 修复（`766264e`）——空 `$$` 公式块里输入第一个字符不再吞掉闭合定界符并把后续区块卷进公式（空围栏防护此前只认 `code` 类型，未随块级公式可编辑一起扩展）；删除路径补上与插入路径同一份写前重解析证明，多行块内整行删除不再把硬换行段落拆成两段、把嵌套列表行变成 setext 标题；删除把普通空格重新留在块尾时按插入路径同一规则在同一笔编辑里改写为 U+00A0，不再产生永不可见的死字节；U+00A0 自愈打通输入法整词上屏与多字符插入路径（此前只认单字符按键，中文输入在每个空格后的词前都会残留一个 U+00A0）。每个形状均有 headless 与真实应用双钉子，且新测试都先在修复前源码/构建上确认会失败。
- 实验：源码内核测试完整性审计与门禁修复——斜杠禁用矩阵镜像第二次过时被修正（`/table`、`/code` 语言变体与 `/math` 已接线，`33f1f90`）；node-view 脚本仍在断言已被解除的 mermaid 预览只读，改为正向逐字节证明（`d236422`）；标题空白的两个 legacy 锁定脚本明确标注 LEGACY LOCK、不得引用为内核证据，并补齐内核侧含可见性断言的对应形状（`b22f5a2`）；mermaid 语言解析器卡死缺陷补上普通（非内核）模式下的直接插入回归（`e7c7694`）；标题 slug 修复落地后，阶段脚本对投影修复次数的断言从「恰好一次」收紧为「零次」（`a0c9f96`）。
- 实验：内核状态指示的只读计数补上确定性钉子——不可映射（参差）表格必须计为 1 个只读块、可映射表格必须计 0，且经突变验证非空洞（此前该分支的断言按映射结果分叉，两个方向都能通过）；`test-diverged-ordinary-save-ui` 在挂载等待失败时不再泄漏 Electron 实例（僵尸进程会占住 CDP 端口，使后续每次运行以同一形态失败）；教程与 `docs/ai-handoff.md` 的内核支持矩阵勘误：块级公式可编辑、`/table` 可用、含硬换行的段落可打字、块尾/标题边界空白的 U+00A0 与实体拼写按模式区分说明。
- 实验：源码内核阶段 3 收尾修复。段落/标题末尾与 ATX 标题开头会被 CommonMark 剥离的空白，现在写成真实的 U+00A0 字符（而不是 `&nbsp;`/`&#x9;` 等实体），一旦不再处于末尾会自愈回普通空格/Tab——普通模式仍写实体拼写，两者只是拼写不同，均解码为同一字符；标题的 `attrs.id`（Milkdown 内部用于锚点的 slug）改为透传而非按 Markdown 字节处理，含标题文档不再在内核模式下每次按键都弹出"暂不支持"提示；在空围栏代码块里输入不再吞掉闭合围栏；`READONLY_CODE_LANGUAGES` 策略性只读名单已删除——预览态与普通态在 PM/CM 两侧结构完全相同，只是可见性不同，因此 Mermaid/LaTeX 代码块现在可以正常编辑；块级公式 `$$..$$` 现在可以直接打字编辑（仍拒绝的只有 `Mod-Enter` 退出与语言切换，各自被对应命令单独拒绝）；斜杠菜单的标题/列表转换（`/h1`–`/h6`、`/ul`、`/ol`）与新建表格/代码块/语言变体/`/mermaid`/`/math` 现在都会经内核提交为源码事务；`/image`、`/divider`、`/task`、`/text` 在该批次仍拒绝（已在后续批次接通，见下一条）；`/quote` 虽然接了内核入口，但唯一可达的调用形态（整块内容就是刚输入的查询文字）必然产生一个零子节点的空引用块，ProseMirror 的引用节点无法承载零子节点，因此该入口架构性地永远失败——引用/取消引用请使用右键菜单，两种模式下都可用。
- 实验：源码内核斜杠菜单收官——菜单上的每一项现在都有内核路由。`/task` 写入 `- [ ] ` 加一个 U+00A0 占位字符（这是重解析后仍是真实 `checked:false` 任务的唯一拼写；每种纯 ASCII 写法都只是普通列表文字），输入正文的第一个字符时在同一笔编辑里删占位、写正文；在输入正文之前保存，磁盘上就是真实任务项、冷重开复选框仍在（普通模式会在保存边界降级为字面 `- [ ]` 文字——为此普通模式的空任务降级分支在内核模式下被关闭，普通模式自身行为逐字节不变）；作者文件里原有的 U+00A0 永不被这样替换。`/divider` 写 `---`（会被解析成 frontmatter 的"文档首块 + 后文已有 `---`"形状回退等价的 `***`）；`/image` 写 `![]()`（空图卡片自带上传/贴链接入口）；`/text` 不写任何字节（删除查询文字本身的纯后缀删除，光标交给文档末尾占位机制）。两条具名位置拒绝，提示均写明补救：插入后光标无处安放（`no-caret-home-after-insert`——在文档末尾插入，或先在下方添加一行文字）与 `/text` 不在文档末尾（`text-needs-document-end`——删掉斜杠命令文字保留原块，或在文档末尾使用 /text）；`/task` 紧邻既有列表的段落仍拒绝（空行不会结束 CommonMark 列表，内核拒绝证明它没写的合并；在列表最后一项按 Enter 续项即可）。另修复：文档以分割线/图片结尾时，点选中该原子后直接打字此前被整体拒绝，现在语义为"在其下方输入"（原子字节不动；替换可先删再打），文档中段被选中的原子打字仍拒绝。
...
- **大表格源码分析保持 revision 有界** — 超长合法 cell 不再因 variadic array append 触发调用栈异常；authored/previous/next 三份 table model 作为当前 editor revision 的一个 working set 复用，避免大型文档刚好越过通用 LRU 字符预算后在每次保存检查中循环重解析。
