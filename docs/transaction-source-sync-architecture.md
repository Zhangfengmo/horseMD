...
`src/renderer/src/lib/source-kernel/` 是 spec（docs/superpowers/specs/2026-08-14-source-authoritative-editor-kernel-design.md）方案 C 的第一阶段纯内核（MarkdownDocument 事务、语法索引、字符映射、结构命令、源码历史、种子化状态机测试，`npm run test:source-kernel`）；UI 集成（投影/Gateway/IME）见后续各阶段计划；现有 `mapPlainTextTransactionsToSource` 通道保持不变。

> **当前覆盖面/阻止矩阵已迁移到 `docs/ai-handoff.md` §5.2d–§5.2g，此处不再维护第二份副本。**
> 本节曾经逐字复制过一份几乎相同的"当前支持 / 仍不支持"矩阵；两份记录各自独立更新，很快就相互漂移——`docs/ai-handoff.md` 那份跟上了后续每一轮修复（阶段 3 语法域、计划六默认启用前置条件、2026-08-18 的 mermaid/LaTeX 可编辑化与斜杠菜单扩展），这一份没有，读者据此可能引用到过时的"仍只读"/"仍整篇降级"结论。为避免今后再次出现同一事实两处维护、只改一处的问题，本文件不再持有内核的功能矩阵——只保留下面这条架构性指针：
>
> - 当前支持 / 单块降级 / 整篇降级 / 显式拒绝的完整、最新矩阵：`docs/ai-handoff.md` §5.2d「源码权威内核（kernel-mode，实验，默认关闭）」及其后 §5.2e/5.2f/5.2g 各条已修复缺陷记录。
> - 各阶段实现细节（模块图、veto 通道、代码块域 CM 桥、CRLF ADR、行内 marks/引用域、阶段 3 语法域、计划六默认启用前置条件）：`docs/superpowers/sdd/2026-08-15-source-kernel-integration/`、`.superpowers/sdd/2026-08-16-source-kernel-codeblock-domain/`、`.superpowers/sdd/2026-08-16-source-kernel-inline-marks/`、`.superpowers/sdd/2026-08-17-source-kernel-stage3/` 各自的计划与任务报告。
> - 性能测量与默认启用前置条件的完整数据：`.superpowers/kernel-performance-assessment.md`。
> - 回归锁：`npm run test:source-kernel`、`npm run test:kernel-headless`、`npm run test:kernel-ui`。
