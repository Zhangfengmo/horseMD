...
- **CRLF 代码块 ADR（2026-08-17 解除收窄；取代 2026-08-16 的 fail-closed 收窄）**：旧收窄把 `lineEnding !== '\n'` 的围栏（CRLF 或单独 `\r`）整块排除出 `charMap` 资格，原因不在投影图自己的数学，而在 vendored `@milkdown/components` 的 `CodeMirrorBlock` 节点视图——它的 CM→PM 位置换算（`forwardUpdate`）没有为 CRLF 做任何修正，CM6 的 `Text.of` 按 `/\r\n?|\n/` 切分、`\r` 字节从不进入 CM 内部模型，于是 CM 位置从挂载起就比真实 PM 偏移少 N（N = 之前经过的 CRLF 换行数）。该缺陷现已由 `src/renderer/src/components/editor-codeblock-crlf.js`（prototype 手术，锁在 `npm run test:codeblock-crlf-ui`）在根上修复，收窄的前提消失，因此 `editor-kernel-projection-map.js` 的 `lineEnding` 门已删除。
  ...
  - **仍然 fail-closed 的残留形态**：（1）CRLF/单 `\r` 块里出现**裸 `\n`** 换行——只有当该块当前文本完全不含 `\r`（CRLF 文档里的单行或空围栏）时才可达，此时桥的 `hasCarriageReturn` 快路径委托给 vendored 实现、只会发出 `\n`；若照旧写入 `\r\n`，PM 与字节就会分歧，每次按键都触发 `verifyPlainTextProjection` 的修复 reconcile（即 P3-4 的 churn 症状），所以直接拒绝（veto + 提示）。（2）LF 块里出现带 `\r` 的换行——桥不可能产生，来源不明，同样拒绝。两者都不影响这些块内的普通文字输入与删除。
  - **回归锁**：`test-kernel-projection-map.mjs` Case 16b/16c/16d（CRLF / 单 `\r` / 引用 CRLF 的可编辑性 + 恒等式）、`test-kernel-gateway.mjs` Case 28/28b/28c/28d（逐字节提交 + 跨行合并删除 + 四种拒绝形态）、`test-kernel-mode-headless.mjs` Case 13（端到端提交、零 `projection-mismatch`、裸 `\n` 拒绝、无全局锁死）、`npm run test:codeblock-crlf-ui` stage K（真实应用：内核模式下编辑 CRLF 围栏、统一 CRLF 落盘、冷重开）。
