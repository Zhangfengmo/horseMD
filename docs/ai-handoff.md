...
当前覆盖：段落/标题文字编辑（含计划三修正的段首 `Enter` 上方插空行、块尾连续 `Enter` 每次新增一行空白语义）、列表族/任务勾选/IME/撤销，以及（计划三，代码块域）**LF 换行围栏代码块**的正文编辑（文字/换行/删除，含引用与列表嵌套前缀）、代码块语言切换、`Mod-Enter` 退出代码块（对 mermaid/LaTeX/CRLF 等只读块同样有效，退出命令本身不碰块内容）、CM 焦点下的撤销/重做（路由到内核历史，不让 CM 自带栈或 prosemirror-history 生效）。仍一律 veto：工具栏格式化、斜杠菜单（含用斜杠新建代码块——创建仍是结构插入，未开放）、右键列表/块类型转换、CRLF/单独 `\r` 换行围栏的代码块编辑（fail-closed 收窄，ADR 见下方链接）、mermaid/LaTeX 代码块内容编辑、表格、图片。测试：`npm run test:source-kernel`（纯内核，含 `code-map` 用例）、`npm run test:kernel-headless`（gateway/reconciler/projection-map/composition 接线测试）、`npm run test:kernel-ui`（模式切换/IME/nodeview/代码块域，四个 CDP 回归）。深度记录见 `docs/superpowers/sdd/2026-08-15-source-kernel-integration/`（计划二）与 `.superpowers/sdd/2026-08-16-source-kernel-codeblock-domain/`（计划三）各任务报告，以及 `docs/transaction-source-sync-architecture.md` 对应小节。
