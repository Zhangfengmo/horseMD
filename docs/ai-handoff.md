...
**已诊断待修（2026-08-22 自主验收测试发现,修复设计已写明）**：**占位符里的首字符若自带结构含义（`-`/`#`/`>`/`1.` 等），提交后引发 `map-refresh-failed` + `projection-mismatch` 级联降级**——最常见触发是「标题末尾回车 → 打 `- ` 开列表」（标题结尾文档的 split-placeholder 会话）。孤行 `-` 即裸列表标记，voucher 按「会成为段落」的收敛假设提交（`/text` 的收敛探针刻意用无结构含义的 `x`），重解析却是 list → 绑图失败 → 块只读 → 空格被吞 → 视图残留字面 `-甲`。同家族的空白变体早已被抓并以具名拒绝收场（gateway `virtualBlock && /^[ \t]/` 守卫，注释记录了同样的 map-refresh-failed+projection-mismatch 尸检）；对 `-` 不能拒绝（Enter 后开列表是 Markdown 最常见手势）。**修复设计**：占位符提交检出「候选重解析在锚点处不是段落」时，接受重构——照常落字节，随后走全量 reconcile-to-parse（`diffReplaceRegions`）+ 平绑图（voucher 消费掉，占位符已成真块），光标落新块内容锚点；裸 `-` 项照常只读、下一个空格由既有 `spellMarkerCompletingSpace` 完成标记、继续打字——整条后续管线都已存在，缺的只是「接受重构」这一步。复现（headless 可转 UI 用例）：`'# 题\n'` 打开 → 内核 → 标题尾 Enter → 键入 `- 甲`。
