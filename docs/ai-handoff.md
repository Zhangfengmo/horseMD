...
**仍一律 veto（与降级无关的显式拒绝）**：mermaid/LaTeX 预览块与块级公式 `$$..$$` 的 `Mod-Enter` 退出与语言切换（块内容本身自 2026-08-18 起可正常打字编辑，见下方 5.2g——只有这两个操作各自被 `exitCodeBlock`/`changeCodeLanguage` 单独拒绝，因为 `$$` 定界符没有信息字符串可写、也没有围栏可续）、块级 HTML 编辑、表格行/列的**拖拽换位**（moveRow/moveCol——表格 UI 仅剩的未路由结构手势；**行列增删与对齐切换已于 2026-08-22 不再 veto**，见下方「2026-08-22 表格结构操作」段。建表更早不拒绝——`/table` 自 5ffae3a 起经内核提交，见下方 5.2g）、图片 caption/ratio（PM 展示态；**已缩放的 image-block 在分类层与 commit 层双重拒绝**——该状态下 raw `alt` 槽装的是数字比例、`title` 槽装的是 caption）、CriticMarkup review markup、mark 内部续打字（"续加粗"手势）、斜杠菜单四项的**位置性**具名拒绝（`/image`、`/divider`、`/task`、`/text` **已于 2026-08-20 接通内核路由**，仅在插入位置给不了光标一个可证明的家时拒绝——见下方 5.2g「2026-08-20 斜杠菜单收官」段。其中 `/text` 的「只能用于文档最后一个块」已于 **2026-08-21** 解除：文档中段走同一个 split-placeholder 会话，只剩具名的 `text-neighbors-would-merge`（删掉该块会让上下两块**同记号**列表合并——合并后两项之间的空行不是根级位置、PM 列表也放不下段落，占位无家可归；或原位打字会被上一段吸收）；**不同记号**的相邻列表 CommonMark 本就不合并，`/text` 在那里一直可用。`/task` 的「紧邻既有列表即拒绝」同日**取消**——`provePredictedListMerge` 预测并逐项核对该合并（唯一接受的差异是 list 自身 tight→loose），只剩邻居是**缩进**列表时的通用拒绝。两项详见 5.2g。`/quote` **已不在本清单**：本节旧版把它记为「永远不可用」——空引用块 `>` 重解析为零子节点的 blockquote、与 PM `block+` schema 块数失配、`requireMap` 必拒——该判断自 `123100f`（2026-08-19，fix(kernel): /quote had never once succeeded）起不再成立：投影图现在像空列表项一样为裸 `>` 合成 createAndFill 填充的空段落（虚拟可编辑 pair，锚在标记后的 raw offset），`/quote` 正常提交 `>` 字节、投影为 blockquote、光标落块内可直接打字（下一击键提交 `>正文`）；`scripts/test-kernel-quote-ui.mjs`（`test:kernel-quote-ui`，自 `b106f95` 起）钉的是**成功**场景。fail-closed 残留：`>  `（标记后两个空格，只有第一个属于标记）配对为只读，不把文字锚到会被段落剥掉的字节后面。右键菜单的「引用/取消引用」依旧两种模式可用，与斜杠入口互不依赖）、右键列表/块类型转换、给**裸 URL** 加高亮（`requireMap` 的 anchor 半边拒绝：autolink-literal 的无 position 回退使结果段落不可映射——写对字节再让用户面对一个打不了字的段落严格更糟）、在两类围栏里按 `Enter` 换行——(a) **当前文本不含 `\r`** 的 CRLF/单 `\r` 围栏（CRLF 文档里的单行或空代码块），(b) 行尾风格**混合**且首个内容行以 LF 结尾的围栏（`buildCodeMap` 的 `lineEnding` 只描述首行，桥却按块主导行尾拼写，二者不等即拒绝）；两类块的普通文字输入/删除均照常，且任何会**劈开 `\r\n` 对**的删除/插入范围一律拒绝（ADR 见下方链接）。（硬换行段落已不在此列：2026-08-18 起可正常打字，见上方 (B) 段末尾的修复说明。）

**2026-08-22 表格结构操作（行列增删 + 对齐）经内核提交**：表格 block-handle UI 实际提供的全部结构手势——边界 `+` 按钮（上/下加行、左/右加列）、行/列拖柄的删除按钮、列对齐按钮组——在内核模式下改写**源码字节**而不再被 veto。接线是「换 run、留 UI」：这些手势都经 6 个 Milkdown 命令（addRow/ColBefore/After、deleteSelectedCells、setAlign）派发，`editor-crepe-setup.js` 的 `routeTableCommandsThroughKernel` 在 `crepe.create()` 之后按同一 slice id 重注册它们（调用时读 `isActive()`，降级页自动回落原实现），路由到控制器新入口 `runTableOperation`——它用 `selectedRect` 从活 CellSelection 读出 PM 行列号，经投影图的 tableCell 配对找到表格的 raw 位置（**这一步同时就是"表格已 zip、PM 行号与 mdast 行号同义"的前置证明**；降级表格无 cell 配对、具名拒绝），再调 `lib/source-kernel/commands/table-ops.js` 的纯命令。纯命令的字节算术全部来自解析器自己的 cell position（**转义管道 `\|` 因此天然不被切开**——唯一手工解析的行是分隔行，而 DELIMITER_RE 证明它不可能含转义；行尾闭合 `|` 另做反斜杠奇偶校验），新空行/空列写 `'|  '`（block-insert 的 TABLE_CELL 约定，emptyCellCharMap 可寻址），对齐只重写分隔行 spec 并保留原有 `-` 数量、同值点击是无字节 noop。证明是本家族的两轴纪律 + 一条表格专属轴：候选重解析后同一起点的表格必须是**预测形状**（行数/列数/align 数组逐项相等），每个**未触及格**的段字节与解码内联签名逐一相等且可映射性不收窄，表格区域外 `outsideSignature`（引用内表格走 through-containers 副本，且要求全部行共享同一行前缀——lazy 引用行拒绝）；结果表格还必须能重建 `buildTableSourceMaps`（table-map.js 新导出的无-PM 半边，buildTableCellMaps 重构为在它之上加 PM zip），光标锚点直接取自候选 map 的目标格 `visibleToRaw(0)`，控制器再以 `requireMap: true` 在提交前证明重建投影图可解析该锚。**保留的具名拒绝**：`table-last-row`（删唯一内容行——GFM 拼得出 header-only 表格但 PM schema `table_row+` 装不下，table-map 会整表降级）、`table-last-column`（删唯一列）、`table-op-unsupported`（无闭合管道/参差行/混合行前缀/越界/多行多列选区删除/表格在列表项内/一切证明失败）。钉子：`test:source-kernel-tableops`（headless，LF+CRLF、引用表、转义管道、字节保真、noop、stale-revision）与 `test:kernel-tableops-ui`（LF+CRLF 真实手势：加行后在新行打字、加列、居中对齐、删行删列、undo/redo、last-row 拒绝 toast、保存与冷重开）。测试期测得一个**与内核无关**的 Crepe 组件怪癖：行柄与首列单元格几何重叠，且组件在任意 wrapper pointer-leave 上武装一个**不可取消**的 200ms 全隐藏定时器 + 迟到的 computePosition 写入，点击可能落在柄消失的瞬间（合成 click 不发生）——真人会再点一次，UI 驱动器同样以"无效果即重试"处理；两种模式同样暴露，非本次改动引入。
