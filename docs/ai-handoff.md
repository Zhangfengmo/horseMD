...
**标题首字符处的 `Delete`/`Backspace`（2026-08-21 具名化）**：这两个键在标题 `parentOffset === 0` 处被 `@milkdown/preset-commonmark` 的 `DowngradeHeading` 抢走（keymap `shortcuts: ['Delete','Backspace']`），对 H1 调 `setBlockType(paragraph)`、其余调 `setNodeMarkup(level-1)`——**根本不是删除**，Chromium 什么也没产生，PM 发出的是一笔跨整个标题节点的结构性 `ReplaceAroundStep`。内核没有该标记改写的字节命令，仍拒绝，但改用专用码 `heading-demote-unsupported`（文案点名手势本身 + 两条出口：先按 → 再 Backspace 删字符；改级别去源码模式）。**勘误**：`writepath-completion-report.md` 第 6 项把成因记成「`extractPlainTextSteps` 拒绝 Chromium 产生的事务」并称 Backspace 在此位置正常——两条都不对（Chromium 没参与；Backspace 同样被抢走，因为内核 Backspace 路由要求 `offset === block.start`，而标题内容起点是标记之后）。配套落地的还有 `describeUnclassified`（`editor-kernel-gateway.js`）：**每一次** `unsupported-input-type` 拒绝都往 `window.__hmKernelDiagnostics` 里推一条 `unclassified-transaction`，带步类型/区间/父节点/slice 形状——正是当初「需要给 app 打桩才能 dump tr.steps」而搁置一整轮的那个信息。钉：gateway 套件 §HD（含 promotion / 块中光标 / 跨级降级三条不得借名的反例）、`test:kernel-heading-whitespace-ui` 步骤 5b（两个键各自的具名 toast + 诊断 + 「先 → 再 Backspace」补救的实证）。
