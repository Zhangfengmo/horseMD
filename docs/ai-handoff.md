...
**2026-08-23(第七批收尾)Computer Use「多级列表拼接」复核 + outdent 改号落地(用户指令「检查是否可修复」,证据 `docs/computer-use-input-verification-2026-08-23.md`)**。Computer Use 记录的 `2. 3.two / 3. 4.nested / 4. 5.out` 编号拼接在两条忠实键盘通道(insertText+结构键 keydown;完整 `keyDown` 带字符——与 CGEvent 等价)× 内核/legacy 四组复刻中**均不可复现**,结合该记录自archived的驱动错焦证据,降级为**疑似驱动伪影**(与 8-22 验尸同构;最终裁决待 Computer Use 驱动修复后重测)。复核发现的真实缺陷已修:**outdent 有序项编号不续**(`Shift+Tab` 后嵌套项自己的 `1.`/`2.` 原样回根级——CommonMark 语义正确、Typora 拼写错)。修复=`outdentListItem` 的 **ORDERED-MARKER RENUMBER**(indent 侧 rescue 的镜像:有序项落到有序父项下续 `parent.number+1`、用父列表定界符;bullet 父项/编号已对时字节不动;同一 `provenNestingOnly` 重解析证明把关,被拒时退回纯剥缩进,改号绝不使 outdent 失败)。注意测试拼写陷阱:`1. 甲\n   2. 乙` **不是**嵌套形状(`   2.` 不能打断段落,是惰性续行)——"编号已正确"钉必须用 `1. 甲\n   1. 丙\n   2. 乙`。钉:`test-source-kernel-indent.mjs` OUTDENT ORDERED-MARKER RENUMBER 节(修复轮真实键盘序列现产出与预期逐字节一致的 `3. out`);通道仿真铁律:CDP `rawKeyDown` 不生成字符(内核 keymap 键照常、字符键会整键丢失),仿真真实键盘必须 `type:'keyDown'` 带 `text`,且标点 virtualKeyCode 不能用 `charCodeAt`(`.` 会算成 46=VK_DELETE 被当 Delete 处理)。
