...
标签页级实验开关（状态栏源码按钮旁 ▾ 菜单，不持久化，仅当次会话）。让 Markdown 源码本身成为编辑权威，而不是富文本回写源码。模块：`editor-kernel-mode.js`（控制器）+ `editor-kernel-gateway.js`（事务分类/提交）+ `editor-kernel-reconciler.js`（提交后投影校验）+ `editor-kernel-composition.js`（IME 挂起）+ `editor-kernel-projection-map.js`（raw offset ↔ PM 位置）+ `src/renderer/src/lib/source-kernel/`（纯内核）。两条 fail-closed 出口：(1) 无法归属的事务批经 `editor-source-transactions.js` 返回 `{veto:true}`，PM 视图完全不变（不调用 `updateState()`）；(2) 初始投影图构建失败则整个标签页降级为完整 legacy 行为（`attachLegacyApi()` 捕获的原始 API 实现接管，`kernel.doc.text` 冻结）。诊断走 `globalThis.__hmKernelDiagnostics`（≤100 条结构化元数据，无正文）。当前只覆盖段落/标题/列表族/任务勾选/IME/撤销；工具栏格式化、斜杠菜单、右键转换、代码块、表格、图片一律 veto。测试：`npm run test:source-kernel`（纯内核）、`npm run test:kernel-headless`（gateway/reconciler/projection-map/composition 的接线测试）、`npm run test:kernel-ui`（模式切换/IME/nodeview 三个 CDP 回归）。深度记录见 `docs/superpowers/sdd/2026-08-15-source-kernel-integration/` 各任务报告与 `docs/transaction-source-sync-architecture.md` 对应小节。
