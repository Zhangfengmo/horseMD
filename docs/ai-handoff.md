...
**2026-08-27（D6）标记内打字被静默吞掉——「续加粗」不是一条手势，是一整族**。
本条同时是一次**范围勘误**：遗留清单把它记成「段落末尾接着打字被吞（4/7）」，
实测（`scripts/probe-mark-run-interior.mjs`，真实 app 逐键）**加粗词中间打字同样被吞**，
只有词首落字——所以它不是"末尾续写"，而是**加粗/斜体/删除线/链接的词内部整体不可编辑**。
根因**不在**光标位置：`probe-trailing-mark-inclusive.mjs` 实测 7 例光标全在元素内部，
区别在 mark 的 `inclusive`——PM 给打出的字符盖上运行段的 mark（strong/emphasis/
strike_through/link 都是 inclusive），gateway `plainSliceText` 拒绝**任何**带 mark 的插入切片；
零宽插入又跳过 `stepRespectsMarkedRuns`，于是死在这里，诊断
`unsupported-input-type` + shape `replace[n,n]@paragraph open0/0 <text>`。
inline code 与 `==高亮==` 当时能落字**仅仅**因为本仓库把这两个 mark 声明为
`inclusive:false`——而这条豁免**只在 PM 真的丢掉 mark 的光标处成立**：A/B 实测
（`probe-code-edge.mjs`，对着 stash 出来的修复前构建跑）证明点在 `<code>` 元素**右边缘**时
inline code 修复前**同样被吞**，此前那条"inline code 不受影响"的观察点的是**段落**右边缘，
是另一个位置——两条观察并不矛盾，是**位置不同**。
**修复不是放宽 `plainSliceText`，而是补一条镜像分类**：明文插入的契约（在运行段边界打字
必须落在定界符**外**，`a **bold**` + X → `a **bold**X`）由 `rawNeutralInsert` 保证，
必须原样保留；带 mark 的插入是它的**镜像**，需要镜像解析器——`visibleToRaw`（gap-BEFORE 表），
`character-map.js` 自己的 ADR 早已写明它把字符落在**闭合标记之内**。于是新增
gateway `extractMarkedTextInsert` + `commitMarkedTextInsert`（步上带 `markedInsert` 标记切换解析器，
其余转义/CRLF/表格结构/空白账本全部复用同一个 `commitPlainTextSteps`）。
**结构证明**：单事务单零宽 ReplaceStep、切片为单个带 mark 文本节点、无换行、
且切片的 mark **恰为它要接入的那个运行段的 mark**——这条不是装饰，是偏移可推导的前提：
gap-BEFORE 表落进的是**在此边界闭合**的那个运行段，所以只认光标**前**的节点；
唯一无前节点的位置是块内容起点（`boundaries[0]` 已越过开标记），只有那里才看后节点。
**字节证明在 mode 侧**（与 `mark-input-rule` 同形）：候选字节经编辑器自己的解析链重解析，
必须与事务即将显示的文档逐值相等，否则**具名拒绝** `marked-insert-unprovable`（新 KERNEL_CODES
+ 中英 i18n + toast，字节与视图都不动）——这条具名正是遗留清单要求的"至少给它一个名字"。
commit **自行重推**形状而非信任调用方（`commitPlainText` 的既有契约，在这里是承重的：
镜像解析器的合法性完全建立在"切片 mark 属于闭合运行段"之上）。
**钉翻转（三处，有据）**：`test-kernel-gateway.mjs` 的 (b) 尾边继承、(e) 运行段内部、
Case 6 块首继承——三条原本钉的就是这个缺陷本身；(b) 的明文半边一字未动，仍钉"明文落在定界符外"。
锁定：**新 `test:kernel-marked-insert`**（无头字节矩阵：四种拼写 × 内部/尾边/块首、明文对照、
mark 不匹配/区间替换/多 mark/含换行四条负控、UNMAPPED 具名）与
**新 `test:kernel-marked-insert-ui`**（真实 app 15 例：strong/em/del/link/code × 尾边+内部 ×
LF/CRLF，高亮作对照仍落在定界符外，落盘字节 + 冷重开 + 0 dialog；已入 `test:kernel-ui`）。
