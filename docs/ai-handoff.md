...
### 5.2j 已知遗留（2026-08-31 记录，未实现）：引用内占位会话废弃时的空引用行回收

用户观察：在引用里回车得到空引用行占位后，若不是再按回车退出、而是**点到引用外**直接写字，会话写入的 `>` 空行字节留在源码里（富文本按 CommonMark 不渲染空引用行，两边「看起来」不一致；语义一致、收尾正确）。legacy 不会累积——它每次序列化都丢空引用行；内核字节权威、不做静默正规化，但「会话自己写的、从未填充的空引用行」按票据学说应在会话废弃时回收。未做的原因：会话在任何别处提交时被 bindMap 隐式结束（调用点遍布），回收需要 revision/历史/reconcile 三方联动的专门设计——不是一个便宜补丁。当前缓解：在空引用行上按回车即可整段回收（0.14.35 的 down-walk），或源码模式手删。实现时注意 materializePlaceholder 已记录 `written {from,to}` span（Backspace 回收用的就是它）。
