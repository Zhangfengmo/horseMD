...
标签页级实验开关（状态栏源码按钮旁 ▾ 菜单，不持久化，仅当次会话）。让 Markdown 源码本身成为编辑权威，而不是富文本回写源码。模块：`editor-kernel-mode.js`（控制器）+ `editor-kernel-gateway.js`（事务分类/提交）+ `editor-kernel-reconciler.js`（提交后投影校验）+ `editor-kernel-composition.js`（IME 挂起）+ `editor-kernel-projection-map.js`（raw offset ↔ PM 位置）+ `editor-kernel-cm-bridge.js`（CodeMirror 内每块动态读写门 + 撤销桥 + `Mod-Enter` 退出）+ `src/renderer/src/lib/source-kernel/`（纯内核，含 `code-map.js`/`commands/code-language.js`/`commands/code-exit.js`）。两条 fail-closed 出口：(1) 无法归属的事务批经 `editor-source-transactions.js` 返回 `{veto:true}`，PM 视图完全不变（不调用 `updateState()`）；(2) 初始投影图构建失败则整个标签页降级为完整 legacy 行为（`attachLegacyApi()` 捕获的原始 API 实现接管，`kernel.doc.text` 冻结）。诊断走 `globalThis.__hmKernelDiagnostics`（≤100 条结构化元数据，无正文）。

当前覆盖：段落/标题文字编辑（含计划三修正的段首 `Enter` 上方插空行、块尾连续 `Enter` 每次新增一行空白语义）、列表族/任务勾选/IME/撤销，以及（计划三，代码块域）**LF 换行围栏代码块**的正文编辑（文字/换行/删除，含引用与列表嵌套前缀）、代码块语言切换、`Mod-Enter` 退出代码块（对 mermaid/LaTeX/CRLF 等只读块同样有效，退出命令本身不碰块内容）、CM 焦点下的撤销/重做（路由到内核历史，不让 CM 自带栈或 prosemirror-history 生效），以及（**计划四，2026-08-16，行内 marks + 引用域**）加粗/斜体/删除线/行内代码的施加与撤销（工具栏、右键菜单、快捷键三条路径一致——新模块 `lib/source-kernel/mark-map.js` + `commands/mark-toggle.js` + gateway 的 `extractMarkToggle` 分类 + `requireMap` 守卫；含多字符行内代码/高亮的文档挂载不再降级）、加粗等标记文字之后段落内其余位置的普通打字（gateway `textblockProfile` 的 gap-aware 放宽）、引用/取消引用（新模块 `commands/quote-toggle.js` 的 `toggleBlockquote`，入口是右键块菜单，段落/标题/列表整块/松散列表均可用，非内核模式同一菜单项行为一致）。仍一律 veto：高亮工具栏按钮（`==` 尚无投影配对映射）、链接、mark 内部续打字（"续加粗"手势）、斜杠菜单（除 `/quote` 外——`/quote` 只在整块内容恰好是刚输入的查询占位文字时可用，对已有正文/列表恒被架构性"空引用墙"拒绝，真正的引用入口是右键菜单）、右键列表/块类型转换、CRLF/单独 `\r` 换行围栏的代码块编辑（fail-closed 收窄，ADR 见下方链接）、mermaid/LaTeX 代码块内容编辑、表格、图片。测试：`npm run test:source-kernel`（纯内核，含 `code-map`/`markmap`/`quote` 用例）、`npm run test:kernel-headless`（gateway/reconciler/projection-map/composition 接线测试）、`npm run test:kernel-ui`（模式切换/IME/nodeview/代码块域/行内 marks+引用域，**五个** CDP 回归）。深度记录见 `docs/superpowers/sdd/2026-08-15-source-kernel-integration/`（计划二）、`.superpowers/sdd/2026-08-16-source-kernel-codeblock-domain/`（计划三）与 `.superpowers/sdd/2026-08-16-source-kernel-inline-marks/`（计划四）各任务报告，以及 `docs/transaction-source-sync-architecture.md` 对应小节。

### 5.2e ⚠️ 高优先级独立缺陷：vendored CodeMirror↔ProseMirror 位置换算会静默损坏 LEGACY 编辑器的 CRLF 代码块

计划三 Task 4 调查中发现，**与 kernel-mode 无关、独立存在、影响当前默认启用的 legacy 富文本编辑器**：vendored `@milkdown/components` 的 `CodeMirrorBlock` 节点视图（`node_modules/@milkdown/components/lib/code-block/index.js` 的 `forwardUpdate`，CM→PM 同步）直接用 CodeMirror 6 自己的内部位置计算 PM step 偏移，没有为 CRLF 做任何修正。CM6 的 `Text.of` 按 `/\r\n?|\n/` 切分文档，`\r` 字节从不进入 CM 内部模型，所以 CM 的内部位置从节点挂载起就比真实 PM 内容偏移少 N（N = 之前经过的 CRLF 换行数）——这个偏移在到达 ProseMirror 层之前无法被检测。**结果：在使用 `\r\n` 换行的围栏代码块里做普通编辑（尤其是跨越已有换行的 Backspace/删除）今天就会把该行换行符从 `\r\n` 静默损坏成裸 `\n`，没有任何用户可见的错误提示**，且这个缺陷从 `code_block` 首次变为内核可编辑（计划三 Task 3）之前就已经存在——它是 legacy 编辑路径自身的问题，不是 kernel-mode 引入的。kernel-mode 一侧已经用 fail-closed 收窄规避（CRLF/单独 `\r` 围栏在内核中整体设为不可编辑，见 5.2d 与 `docs/transaction-source-sync-architecture.md` kernel-mode 代码块域小节的 ADR），但 **legacy 路径本身尚未修复**，需要独立专项跟进（补丁/包装 vendored 依赖自己的 CM↔PM 位置数学，或设计另一套安全契约）。完整调查证据见 `.superpowers/sdd/2026-08-16-source-kernel-codeblock-domain/task-4-report.md`「Fix-review round」。
