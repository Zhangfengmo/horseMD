...
**（计划五，2026-08-17，阶段 3 语法域）新增**——组织原则是**先治降级、再扩编辑**（数学/行内 HTML/高亮三类形状此前让**整篇**文档拒绝建图，属于"整个标签页降级回 legacy"而非"该域不可编辑"）：含行内 `$x$` 与多行 `$$…$$` 的文档正常挂载（内核链加 `remark-math`，inline math 是 width-1 atom；block math 当时仍只读，2026-08-18 起可编辑，见下方 5.2g）；含 `<span>…</span>` 等行内 HTML 片段的文档正常挂载（`lib/source-kernel/inline-html.js` 是编辑器链与内核链**共享的同一份**合并规则——编辑器侧 `remarkMergeInlineHtml` 合成的节点无 position，不得进内核链；片段本身是原子，片段外的位置可编辑、Enter/Backspace 可用）；`==黄色高亮==` 段落可映射可编辑且工具栏黄色按钮解封（`lib/source-kernel/highlight-syntax.js`，**ADR：不是 micromark 扩展**——实测编辑器根本不在 parse 期识别 `==`，而是在 parse 完成后对 mdast text 节点的**解码值**跑正则，一致性由 `test-source-kernel-highlight-consistency.mjs` 的 55 形状 + 4000 份随机对抗文档逐字节锁死）；**表格单元格文字可编辑**（`lib/source-kernel/table-map.js`：PM 4 层 ↔ mdast 3 层在表内单独 zip，主 zip 里整表仍占一槽；分隔行在 mdast 里无节点、只作证据推出、绝不写入；`Tab`/`Shift-Tab` 明确路由到 `goToNextCell`，零字节写入）；图片 `alt`/`src`/`title` 的源码改写命令就绪（`commands/image-attrs.js`，**但目前没有 UI 派发 alt/title**，用户可达的只有"源码里已有 `![]()` 时填入 src"这一条；**kernel/image-caption 起图注输入框成为第二条用户可达路径**——caption→title 槽，详见下方 5.2d 清单的 ratio/caption 项）；链接的 wrap/改 URL/移除全部走源码事务（`commands/link-toggle.js` + gateway 新分类 `link-edit`，识别 LinkTooltip 在**一个 tr** 里派发的 `removeMark`+`addMark`(+`insertText`) 混合形态——正是 `extractMarkToggle` 明确拒绝的形状，故单独分流）。

**残留限制分两种机制，不得混为一谈**（影响范围完全不同）：

**(A) 整图 null → 整个标签页降级回 legacy**（`attach-unmappable`，`attachLegacyApi()` 的原始 API 全面接管）。成因永远是**块序配对失败**（块数/类型不匹配），这仍是全局 fail-closed，Task 2.5 的单块降级没有也不应放宽它。**曾经列在这里的三种形状已全部消除**（(2) 于 2026-08-20 `6374848`，(1)/(3) 于 2026-08-22）——每一种都成为**至多只读叶、绝不整篇降级**，且各带负控证明 fail-closed 没有放宽：(1) 独占一行的 `$$x$$`（编辑器链 parse 前跑 `normalizeDisplayMath` 改写成三行 → PM 是 LaTeX `code_block`，内核**刻意**持有原始字节 → mdast `paragraph`）：`isNormalizedDisplayMathPair`（projection-map）在**段落整个 raw span 匹配 `/^\$\$[^\n]*\$\$[ \t]*$/`**（恰是 normalize 的整行规则；行尾空格实测在 span 内、行首缩进实测在 span 外）且 PM 语言为 LaTeX 时把该槽配成**只读叶**——一行 raw 对三行 PM，字符映射不存在，编辑必须继续拒绝（CM 桥按 `charMap` 为 null 拒键，`Mod-Enter`/语言切换走各自命令拒绝），文档其余块照常编辑；带尾随文字/懒续行/非 LaTeX 语言仍整图拒绝。(2) 列表项内的块级数学或**任意**代码围栏（PM 的 `list_item` content model 经 `createAndFill` 插入填充 paragraph → 块数失配，**与数学无关**）：`flattenMd` 的 `syntheticLeadingItemParagraph` 为填充段落合成配对槽——填充段落只读（无字节可写、非 virtual），围栏本体照常走 `buildCodeMap`，**但列表缩进前缀今天证明不了 → 围栏自身也是只读叶**（诚实钉在 Case M6；code-map 学会列表前缀之日翻转该断言）。(3) 相邻的根级 `<div>` 兄弟（编辑器链跨空行合并成一个原子、内核保持 N 块）：配对环节允许 N 个连续**根级** mdast html 块被一个 PM html 原子按**单个只读叶**消费——run 判定用编辑器自己的 `inlineHtmlRunAt`（两链共享同一份实现，root 的 `breakHtmlCuts=false` 逐参数一致），且 PM 原子 `attrs.value` 必须与 run 的拼接值**逐字节相等**（拼接值就是编辑器合并节点携带的 value，相等即证明合并正是这样发生的）；value 不符 → mdast 块消费不完 → 整图照旧拒绝。**仍整篇降级的残留**：引用/列表项**内**的同款 html 合并（run 证明未沿容器前缀论证，钉为 Case H9 残留负控）。钉子：`test-kernel-projection-map.mjs` Case P6（形状 1，含三条负控）/ Case M6（形状 2 + 围栏只读钉）/ Case H9（形状 3，含 value 负控与容器残留）+ H10（对照）；`test-source-kernel-index.mjs` `$$…$$` 独立小节（内核侧解析真相不变）；真实 app 三形状同文档附着 + 旁路打字 + 字节精确保存：`test:kernel-attach-shapes-ui`（非空洞性用修复前投影图重建验证过：彼时恰在 `attach-unmappable` 断言处失败）。**曾经的第四种形状 YAML frontmatter 已在计划六 Task 2 消除**：内核 unified 链补挂 `remark-frontmatter`（与编辑器链同一默认 `yaml` 预设，纯 micromark/mdast-util 扩展，不引入 transform，仍满足"只 parse 不 runSync"），`PM_TO_MD` 新增 `frontmatter: ['yaml']`。**只是不再降级，不承诺可编辑**：`frontmatter` 是 PM atom，`isTextblock` 为假 → `charMap: null`，与表格/块级数学/块级 HTML 同为只读叶；frontmatter 内部偏移在 `blockAt` 里也解析不到块（`yaml` 不在 `BLOCKS`），结构命令一律 fail-closed。文档中段真正的 `---` 分隔线不受影响（预设只在文首匹配，有负控钉住）。钉子：`test-source-kernel-index.mjs` frontmatter 小节 + `test-kernel-projection-map.mjs` F1–F5。

**超过 `CHUNK_THRESHOLD`（120 000 字符）的文档：2026-08-21 起可以正常附着**（此前无论多大都不行）。根因仍如旧文所述：`appendChunks` 对每个 ~40 KB 分块**独立解析**后追加，内核整篇解析一次，两者在真实内容上确实不同（实测本仓库 docs/ 拼成的 262 KB 文档：整篇 1585 块 vs 分块 1572 块，首个分歧在第 647 块；典型形状是"空行分隔的两段列表项"——整篇解析为一个 loose list，分块解析为两个 list）。**镜像方案 (d) 依旧被拒绝**（理由见下方保留原文，`editor-kernel-mode.js` 的 ADR 也原样保留：内核绝不学习"分块"这个概念）。新方案是**方案 (b)：在载入结束时把视图修复成整篇解析形态，再照常附着**——`repairChunkedProjection`（`editor-kernel-mode.js`，attach 路径）用编辑器自己的 parser（就是热路径的 `safeParse`，不新增第二个解析入口）重新解析 `kernel.doc.text`，用 `diffReplaceRegions`（`editor-kernel-reconciler.js`）只替换真正分歧的区域，随后 `attachAfterCreate` 跑的是**普通的整篇配对**。必须是多区域 diff：`diffReplaceRange` 只框住"第一个"和"最后一个"分歧并吞掉中间一切，在 646 KB 的 docs/ 拼接文档上是**全文的 90.8%**（等于几乎每个 node view 都重挂，含每个 CodeMirror），多区域答案是 **7.8%**；区域搜索是"逐步扩大半径 + 用下一对节点确认"（重复段落骗不到 resync），1:1 同 markup 的区域再向内收窄一层；全程**不查询任何分块边界**，因此没有"编辑是局部的"这类假设。时机与安全性：修复跑在 `appendChunks` 的 `onChunksApplied` 钩子里，即最后一块追加之后、恢复可编辑之前——载入期编辑器本来就是只读的，所以没有用户编辑可丢；控制器在 reconcile 之后强制 `view.state.doc.eq(parsed)`，不成立就 fail-closed。**实测一次性成本**（真实 app，200 KB 语料）：修复 235–604 ms + 附着 106–159 ms，合计 371–733 ms，一次性；headless（真实解析链）400 KB 为 ~435 ms + ~280 ms。远低于"超过 1.5 s 就必须挪出载入路径"的门槛，因此保持同步在载入尾部。**两类拒绝仍然各自具名**：整篇重解析失败 → `kernelMode.chunkRepairFailed`（reason `chunk-repair`）；修复成功但整篇形态本身仍配不上 → `kernelMode.unmappableChunked`（reason `chunked`，文案已改写成"已重新合并但仍无法配对"）。顺带修掉两个载入期缺陷：分块追加改为 `addToHistory: false`（此前打开大文档后第一次 Ctrl-Z 会撤掉一整块 40 KB 追加），以及内核开关在载入期间**等待**而不是序列化半截文档（`kernelMode.toggleDuringLoad`）。钉子：`scripts/test-kernel-chunk-attach.mjs`（headless，Case 0 就是修复前的拒绝，作为非空洞对照）与 `test:kernel-chunk-attach-ui`（LF + CRLF，真实 app 打开 200 KB 文档→附着→首/中/尾输入→保存→冷重开）。以下是被拒绝的镜像方案原文，保留备查：**计划的镜像方案 (d) 被明确拒绝**，理由不是偏移算术（`chunks.join('\n') === 原文` 与 `slice` 性质 LF/CRLF 都已复验并写进 headless 测试），而是：(1) `safeParse` / `verifyPlainTextProjection` / `reconcileProjection` 全部按**整篇**解析并据此修复视图，只镜像内核自己的解析会让第一次键入把视图对齐成整篇形态、下一次 `bindMap` 立刻失配 → 地图为 null、文档彻底不可写；(2) 分块边界是按**当前文本**重算的（"累计 40 000 字符后的第一个空行"），一次编辑挪动边界就会改变块序列而 PM 保持旧形态，映射会随打字在可用/不可用之间翻转——那是"编辑是局部的"这一断言，不是证明。
...
**(B) 配对成功、该块 charMap 置 null → 只读叶，文档其余部分照常编辑**（提示 `kernelMode.blockReadOnly`）。成因是**块内单元数与 PM 内容尺寸不符**，即 Task 2.5 通解覆盖的那一类：(1) 红/蓝高亮 `<mark class="hm-hl-…">`（它是行内 HTML：内核 1 个 atom vs PM 的 N 字符标记 run，N>1 → **该块**只读——钉在 projection-map Case P3c，并兼任 `test-kernel-mode-headless` Case 17 的"一个不可证块不拖垮整篇文档"钉子）；(2) 含 `<br>` 或转义 `\|` 的表格单元格 → **格级**降级，同表其余格照常可写；(3) 参差/无法证明的表格 → **表级**降级，表格以外的块照常可写。（曾有第 (4) 类——含**硬换行**的段落被 gateway `textblockProfile` 整段拒绝——已于 2026-08-18 由 `6560df5` 修复，见本段末尾。曾有第 (5) 类——**CRLF/孤立 CR 文档里的软换行段落**整块只读，`textUnits` 把 `\r\n` 对的 `\r` 记成独立 char 单元、每个软换行让 `visibleLength` 比 PM 的 `content.size` 多 1，日常受害形状是列表项续行——已于 2026-08-21 由**行尾加宽**修复：一个行尾（任何拼写）就是**一个** width-1 `linebreak` 单元，raw 跨下整对字节加续行前缀，组合矩阵 D2 家族整族消失；见下方 5.2g「2026-08-21 CRLF 软换行加宽」段。）**计划六 Task 1（2026-08-17）已放宽行内图片 / 行内公式 / 行内 HTML / 脚注引用**：这些段落现在可正常打字；Task 1b 进一步允许**整体删除/替换原子**（选中图片按 Backspace，或连同前后文字一起删），因为解析出的 raw 区间恰好是该原子自己的字节——PM 侧由 `stepRespectsAtoms` 证明范围不会只覆盖原子的一半，raw 侧由 `commitPlainText` 的 `rangeSplitsAtomUnit` 在写入前再证一次。仍被拒绝的是：部分覆盖原子的步骤（今天的原子都是 nodeSize 1，构造不出来，但守卫按通用区间写），以及**带 mark 的原子**（链接图片 `[![x](y)](url)`：`[` 与 `](url)` 不属于任何单元，`stepRespectsMarkedRuns` 只走 text 子节点看不见它，单删图片会留下孤立定界符）。**硬换行的整段拒绝已于 2026-08-18 解除**（`6560df5`）：历史原因是它的 raw 跨度止于行尾、下一行的续行前缀（缩进或 `> `）不属于任何 charMap 单元，紧随其后的插入点会落在空隙之前（`> a  \n> b` 处打字会把 `> ` 降级成正文）；现在 `character-map.js` 的 `hardBreakUnitEnd` 把续行前缀折进 break 自己的单元并逐字节证明（只允许折到下一兄弟节点的起始 offset，且跨过的每个字节都必须是续行前缀字符），单元因此在硬换行两侧连续铺满，含硬换行的段落可正常打字——钉在 `scripts/test-kernel-hardbreak-ui.mjs`、扩展后的 `test-kernel-gateway.mjs` 与 `test-source-kernel-charmap.mjs`。

**标题首字符处的 `Delete`/`Backspace`（2026-08-21 具名化 → 2026-08-22 落地为字节命令）**：这两个键在标题 `parentOffset === 0` 处被 `@milkdown/preset-commonmark` 的 `DowngradeHeading` 抢走（keymap `shortcuts: ['Delete','Backspace']`），对 H1 调 `setBlockType(paragraph)`、其余调 `setNodeMarkup(level-1)`——**根本不是删除**，Chromium 什么也没产生，PM 发出的是一笔跨整个标题节点的结构性 `ReplaceAroundStep`。**2026-08-22 起「标题降级」是内核自己的字节命令**（用户报告：文档尾部的空 H1 完全删不掉）：`commands/block-type.js` 新增 `demoteHeadingAtCaret`（正是该文件头部注释预留的「快捷键入口出现时建在这里」的那个通用转换器），由 `editor-kernel-mode.js` `commitHeadingDemote` 在结构路由答 `not-structural` 的 Backspace/Delete 上先行尝试（`looksLikeAtxContentStart` 便宜预筛，热路径无 parse）。三种形状：H_n(n≥2) 删标记 run 里一个 `#`；有内容的 H1 删除整个 ATX 前缀（重解析证明剩余字节恰为同内容段落——`# 1. 甲`/`# # 甲`/`# 甲` 紧贴下一行等会变结构的形状具名拒绝，绝不盲写或造转义）；**空 H1 委托 `/text` 机制**（空标题的内容起点恰是块末尾，`insertBlockFromQuery target:'text'` 的守卫天然通过，删块字节 + 光标乘既有 doc-end/split 占位符会话——`applyRoutedBlockResult` 即从 `runInsertBlockFromQuery` 提出的共用尾部）。**顺带修了一个更老的静默 bug**：`trailingPlaceholderPair()` 只凭 `virtual`+空 textblock+文档尾判定「尾部占位符」，而空 ATX 标题的 pair 同样 virtual（真字节上的纯锚点对）——Backspace 被「占位符边缘」逻辑吞掉（trim 证明失败 → 只挪光标），这正是空 H1 删不掉的直接死因；判别式补上 `mdBlock: null`（两种真占位符都无字节，空标题有）。网关的 `heading-demote-unsupported` 具名拒绝**保留**为安全网（嵌套在引用/列表里的标题仍走 PM 事务→分类拒绝），文案已改写为剩余现实（会变结构/不在顶层 + 两条出口）。钉：`test-source-kernel-blocktype.mjs` §9（三形状 + 具名拒绝 + not-structural 契约 + CRLF）、`test:kernel-heading-demote-ui`（LF+CRLF：H2→H1 撤销重做、H1→段落即刻可打字、拒绝零字节、空 H1 经占位符删除、Delete 同手势、save+冷重启）。**勘误**：`writepath-completion-report.md` 第 6 项把成因记成「`extractPlainTextSteps` 拒绝 Chromium 产生的事务」并称 Backspace 在此位置正常——两条都不对（Chromium 没参与；Backspace 同样被抢走，因为内核 Backspace 路由要求 `offset === block.start`，而标题内容起点是标记之后）。配套落地的还有 `describeUnclassified`（`editor-kernel-gateway.js`）：**每一次** `unsupported-input-type` 拒绝都往 `window.__hmKernelDiagnostics` 里推一条 `unclassified-transaction`，带步类型/区间/父节点/slice 形状——正是当初「需要给 app 打桩才能 dump tr.steps」而搁置一整轮的那个信息。钉：gateway 套件 §HD（含 promotion / 块中光标 / 跨级降级三条不得借名的反例；分类本身不变——它仍是嵌套形状的安全网）、`test:kernel-heading-whitespace-ui` 步骤 5b（2026-08-22 起改钉与本域的交集：NBSP 开头的 H1 两个键各自降级成功、账本化 U+00A0 逐字节保留、Mod-z 恢复）。

**验尸更正（2026-08-22 自主验收测试）**：曾在此记录「占位符里首字符带结构含义（`-` 等）引发降级级联」为待修缺陷——**判决推翻，真实链路健康**。标题末尾回车 → 打 `-`（重构成裸项，`map-refresh-failed`+`projection-mismatch` 是记录在案的日常瞬态）→ **真实空格键**（keydown → `spaceHandler` → `spellMarkerCompletingSpace`）→ 正文，全程正确、字节与视图收敛（实测 `[h1, ul:甲]`）。当初的"损坏"来自测试驱动器：`typeTextLikeUser` 走 `Input.insertText`，**不产生 keydown**，补全 keymap 永远收不到空格 → 空格落进只读裸项被拒。**测试铁律**：凡语义依赖 keymap 的键（空格补全、Enter、Tab、Backspace/Delete、Mod-*），必须用 `pressKey`（真实 keydown）驱动；`typeTextLikeUser` 只用于纯文本字符。真实世界的残余暴露面：不经 keydown 注入空格的输入路径（某些 IME 上屏含空格的组合），接受为已知边界。

**仍一律 veto（与降级无关的显式拒绝）**：mermaid/LaTeX 预览块与块级公式 `$$..$$` 的 `Mod-Enter` 退出与语言切换（块内容本身自 2026-08-18 起可正常打字编辑，见下方 5.2g——只有这两个操作各自被 `exitCodeBlock`/`changeCodeLanguage` 单独拒绝，因为 `$$` 定界符没有信息字符串可写、也没有围栏可续）、块级 HTML 编辑、表格拖拽换位中**表头行的参与**（2026-08-30 起 moveRow/moveCol 已路由内核：行换位是 body 区整行置换、列换位是每条物理行的分段重排（对齐规格随列走），均经 proveAndPackage 逐格证明；上游 prosemirror-tables 对表头无任何保护、会把 body 行静默改标为表头，内核不抄——from/to 涉及行 0 时具名拒绝 `table-header-row`。同批根修 reconcileProjection：diff 两端落在不同表格行的单元格内部时扩至整表节点替换（widenReplaceForTables），否则深开口 slice 跨行拟合出参差行、被 fixTables 补空列——实测字节对、视图多一列；安全形状（增删行、格内编辑）保持最小 diff 不重挂组件。钉子：tableops 无头 §11 + tableops-ui F2（真实 DragEvent 流，LF+CRLF）+ reconciler Case 12–15。**行列增删与对齐切换更早（2026-08-22）已不再 veto**，见下方「2026-08-22 表格结构操作」段；建表 `/table` 自 5ffae3a 起经内核提交，见 5.2g）、图片**无图注时**的拖拽缩放（2026-08-30 起 ratio 已路由内核：`setImageRatio` 写 legacy 自己的多槽拼写——alt 槽改数字比例、图注挪进/保持在 title 槽，复用 setImageAttrs 全套扫描/重解析/转义证明；已缩放态的再缩放与恢复 1x 也走同路（恢复 1x 写 `![](url "图注")`，与 AttrStep 后的视图 attrs 逐项一致）。唯一剩下的具名拒绝就是**无图注缩放** `image-resize-unsupported`：`![1.50](url)` 无 title 会重解析成「图注为 1.50 的未缩放图」——字节格式本身装不下，legacy 在这里是静默丢失缩放，内核改为具名拒绝并提示先补图注。钉子：commands 无头 ratio 节、gateway I4/I5/I6、mode-headless 22d、`test:kernel-image-caption-ui` 第 5 节（真实拖拽→数字 alt 拼写+落盘+冷重开，LF+CRLF）。**caption 已不在本清单**：未缩放 image-block 的图注编辑自同批起走 `setImageAttrs({caption})` 提交进 markdown **title 槽**——这正是 legacy 自己的字节方案，`editor-image-markdown.js` 解析 `caption: title || alt`、序列化把 caption 写进 title；命令在 mdast 双轴之上多跑一道 **schema 投影证明**（parse runner 的纯镜像），拦下 mdast 轴看不见的两族并各自具名：数字 alt 陷阱——`![2](x)` 本按未缩放解析，写入 title 会翻转成 legacy 缩放读法、图片瞬间 2 倍——拒 `image-caption-scaled`；带 alt 时清空图注无字节拼写（去掉 title 后 alt 会顶回图注槽显示）——拒 `empty-image-caption-unrepresentable`。**已缩放的 image-block 图注仍拒绝**（该状态下 raw `alt` 槽装的是数字比例、`title` 槽装的是 caption），且从双重变**三重**——分类层、commit 层、命令层各自复推序列化器同一判据，三处都答 `image-caption-scaled`。钉子：`test-source-kernel-commands.mjs` caption 节、`test-kernel-gateway.mjs` I4/I4b/I5/I6b、`test-kernel-mode-headless.mjs` 22d/22d2、`test:kernel-image-caption-ui`（LF+CRLF 真实光标点进图注输入框打字/清空/拖拽缩放）与 stage3-ui 第 8b 步（图注提交零 projection-mismatch）。一个诚实的观察，记录而未实现：**已缩放图注的非空改写其实可证**（title 槽本就装图注，重写它保持 legacy 读法与 ratio 不变），但按任务指示保留拒绝——清空侧确实不可表达（`![1.50](x)` 无 title 会翻回未缩放读法、缩放即毁），先保守）、CriticMarkup **替换建议标记的插入**（`{~~旧~>新~~}`：其 `~~` 字节对内核链是 GFM 删除线、对编辑器链经 `remarkReconstructSubstitution` 合并回字面文本，两链在该块上必然失配，具名拒绝 `review-substitution`；**已含**替换标记的段落同因降级为只读 pair——审阅域其余操作（新增/删除/高亮+批注的选区包裹、卡片 Done/Delete/Edit、全部接受/拒绝）**2026-08-22 起已接通内核**，见 5.2g 审阅域段）、审阅包裹的非纯文本选区（选区碰到 mark 定界符/原子/实体/转义/换行——具名 `review-plain-selection`，跨行沿用 legacy 的 `review.inlineOnly` 文案）、mark 内部续打字（"续加粗"手势）、斜杠菜单四项的**位置性**具名拒绝（`/image`、`/divider`、`/task`、`/text` **已于 2026-08-20 接通内核路由**，仅在插入位置给不了光标一个可证明的家时拒绝——见下方 5.2g「2026-08-20 斜杠菜单收官」段。其中 `/text` 的「只能用于文档最后一个块」已于 **2026-08-21** 解除：文档中段走同一个 split-placeholder 会话，只剩具名的 `text-neighbors-would-merge`（删掉该块会让上下两块**同记号**列表合并——合并后两项之间的空行不是根级位置、PM 列表也放不下段落，占位无家可归；或原位打字会被上一段吸收）；**不同记号**的相邻列表 CommonMark 本就不合并，`/text` 在那里一直可用。`/task` 的「紧邻既有列表即拒绝」同日**取消**——`provePredictedListMerge` 预测并逐项核对该合并（唯一接受的差异是 list 自身 tight→loose），只剩邻居是**缩进**列表时的通用拒绝。两项详见 5.2g。`/quote` **已不在本清单**：本节旧版把它记为「永远不可用」——空引用块 `>` 重解析为零子节点的 blockquote、与 PM `block+` schema 块数失配、`requireMap` 必拒——该判断自 `123100f`（2026-08-19，fix(kernel): /quote had never once succeeded）起不再成立：投影图现在像空列表项一样为裸 `>` 合成 createAndFill 填充的空段落（虚拟可编辑 pair，锚在标记后的 raw offset），`/quote` 正常提交 `>` 字节、投影为 blockquote、光标落块内可直接打字（下一击键提交 `>正文`）；`scripts/test-kernel-quote-ui.mjs`（`test:kernel-quote-ui`，自 `b106f95` 起）钉的是**成功**场景。fail-closed 残留：`>  `（标记后两个空格，只有第一个属于标记）配对为只读，不把文字锚到会被段落剥掉的字节后面。右键菜单的「引用/取消引用」依旧两种模式可用，与斜杠入口互不依赖）、右键列表/块类型转换、给**裸 URL** 加高亮（`requireMap` 的 anchor 半边拒绝：autolink-literal 的无 position 回退使结果段落不可映射——写对字节再让用户面对一个打不了字的段落严格更糟）、在两类围栏里按 `Enter` 换行——(a) **当前文本不含 `\r`** 的 CRLF/单 `\r` 围栏（CRLF 文档里的单行或空代码块），(b) 行尾风格**混合**且首个内容行以 LF 结尾的围栏（`buildCodeMap` 的 `lineEnding` 只描述首行，桥却按块主导行尾拼写，二者不等即拒绝）；两类块的普通文字输入/删除均照常，且任何会**劈开 `\r\n` 对**的删除/插入范围一律拒绝（ADR 见下方链接）。（硬换行段落已不在此列：2026-08-18 起可正常打字，见上方 (B) 段末尾的修复说明。）
...
**已知落差(2026-08-26 记录在案,本轮不修——两条都不影响字节,只影响显示)**:①**源码忠实序号只在编辑器内生效,导出不带**。`68a0b38` 的 `sourceOrdinalPlugin` 把作者字节里的号画在 ProseMirror 的自动序号上(实测字节 `1. a / 5. b / 9. c` 在编辑器里显示为 1、5、9),但 `editor-pdf-content.js` 的 `stripEditorOnlyForExport` 会把 `.label-wrapper` 整个从导出快照里剥掉,所以 **PDF / HTML 导出渲染的是顺序号**(1、2、3),与编辑器所见不同。这条分叉本身可能正是想要的(导出目标沿用 GitHub 式顺序号),但在此之前**没有任何文件写下过它**,也没有任何钉;Pandoc 导出不走该快照(直读 Markdown 字节),不受影响。②**两条序号显示装饰作用在同一个 DOM 节点,交互无人讨论**。`app.css` 的 `.hm-undecided-ordinal`(未决态灰显,`opacity: 0.35`,第二阶段任务②)与 `.hm-source-ordinal`(源码号替换,`visibility: hidden` + `::before` 重画,`68a0b38`)选中的都是 `li > .label-wrapper > .label.ordered`。一个列表项同时带上这两个 class 时的观感(在已被隐藏的 label 上再调 opacity;`::before` 画出的源码号是否跟着灰)既无实测也无钉。背景与裁决见 `docs/typing-policy-chokepoint-adr.md` 补记「源码忠实序号与 outdent 改号」。
...
**超阈值诚实收口（计划六 Task 5）**：超过 `CHUNK_THRESHOLD`（120 000 字符，`editor-chunked-parse.js:15`）的文档，`appendChunks` 对每个 ~40 KB 分块**独立解析**后追加，内核对整篇解析一次，两者在真实内容上确实可能不同（实测本仓库 docs/ 拼成的 262 KB 文档：整篇 1585 块 vs 分块 1572 块）——这不是 bug，是两种解析策略的固有差异。计划原本把"内核镜像同一套分块解析"（方案 (d)）列为首选，**该方案在实现阶段被明确拒绝**：偏移算术本身是精确的，但 `safeParse`/`verifyPlainTextProjection`/`reconcileProjection` 全部按整篇解析并据此修复视图，只镜像内核自己的解析会让第一次键入把视图对齐成整篇形态、下一次 `bindMap` 立刻失配、地图变 null、文档彻底不可写；而且分块边界按当前文本实时重算，一次编辑就可能移动边界，使映射在可用/不可用之间随打字翻转——"编辑是局部的"是一个假设，不是证明。最终采用的是方案 (c)：**依旧照常尝试附着**（两边解析恰好一致的大文档继续可用，逐块证明一个不少），一旦失败，提示改为专用文案 `kernelMode.unmappableChunked` 并在诊断里带 `chunked: true`——不再和普通"无法配对"混为一谈，**该状态已于 2026-08-21 结束**——见下方「2026-08-21 超阈值附着（方案 (b)：载入尾部整篇修复）」段：视图在载入结束时被修复成整篇解析形态，附着照常进行，120 KB 以上的文档现在可以进入内核模式编辑；方案 (d) 的拒绝理由与本段其余部分一字不改地成立（内核仍然不知道"分块"是什么）。

**2026-08-21 超阈值附着（方案 (b)：载入尾部整篇修复）**：`CHUNK_THRESHOLD`（120 000 字符）以上的文档终于可以运行源码内核。`Editor.jsx` 给 `appendChunks` 传入新的 `onChunksApplied` 钩子——最后一块追加之后、`finally` 恢复可编辑之前触发，钩子调用 `kernelController.repairChunkedProjection({ yieldTurn })`：① 用 `safeParse`（热路径同一入口，带 `withTrailingParagraph`/`withHeadingIds` 对齐）整篇重新解析 `kernel.doc.text`；② `diffReplaceRegions(view.state.doc, parsed)` 走顶层子节点做"扩半径 resync"（用下一对节点确认，避免重复段落骗过对齐），1:1 且 markup 相同的区域再用 `diffReplaceRange` 向内收窄一层；③ `reconcileProjectionRegions` 把所有区域**倒序**放进**一个**事务（原始坐标因此无需映射），带 `sourceProjection` + `addToHistory:false`；④ 强制 `repaired.eq(parsed)`，不成立即 fail-closed（`chunkRepair.failure = diverged`）。随后 `finishInitial` 照常调用 `attachAfterCreate`，跑的是**普通的整篇 `buildProjectionMap`**——没有任何一处代码把分块边界当作证据。**为什么必须多区域**：分块文档的分歧散布在每个边界附近，单区域的 `diffReplaceRange` 会把首尾之间整段吞掉——646 KB 的 docs/ 拼接文档实测 **90.8%** vs 多区域的 **7.8%**；合成 chunk-trap 语料（每个边界都横跨一个 loose list，比真实文档更严苛）在 130–400 KB 上是 **6.2–7.2%**，顶层节点 `===` 身份保留 **>99.3%**，**每个 code_block 的节点身份 100% 保留**（CodeMirror 是急切挂载的，重挂就是 400 KB 卡死）。**实测一次性成本**：真实 app 200 KB — 修复 235–604 ms + 附着 106–159 ms（`chunk-repair` / `chunk-attach` 两条诊断自带 `ms`，不是从 long-task 轨迹里猜的：整次重载本来就阻塞 ~8 s，把那个数算到修复头上会差一个数量级）；headless（真实解析链，M4 Pro）130 KB ~138+82 ms、200 KB ~200+125 ms、400 KB ~435+280 ms。低于"超过 1.5 s 就改成 idle 调度"的门槛，故保持同步在载入尾部。**诚实兜底仍在**：整篇重解析失败 → `kernelMode.chunkRepairFailed`；修复成功但整篇形态仍配不上 → `kernelMode.unmappableChunked`（两者 `describeKernelStatus` 各有 detailKey）。实测确实存在后者的真实标本：docs/ 全量拼接（646 KB）的**整篇**解析本身就配不上，与分块无关。**顺带修掉的两个载入期缺陷**：(1) 分块追加此前进了 undo 历史，打开大文档后第一次 Ctrl-Z 会撤掉一整块 40 KB 追加——现已 `addToHistory: false`；(2) 载入期间点内核开关会把**半截文档**序列化并写回 `tab.content`，重载后文件被截断——现在载入期直接以 `kernelMode.toggleDuringLoad` 拒绝并让用户稍后再试。另外，内核开关对**干净标签页**不再走 verified flush（`tab.content` 就是那份字节）：分块文档按构造过不了那道闸门，此前每个超阈值文档点开关都只会得到 `kernelMode.toggleFailed`，内核根本够不着。**一个测出来但没在本轮修的缺陷**：同一道 verified 闸门意味着**分块载入的文档在 legacy 模式下根本存不了盘**——在 200 KB 文件里敲一个字符再按保存，会弹出「无法安全合并回 Markdown 源码，是否另存恢复副本」并且原文件纹丝不动。这是既有缺陷、与内核模式正交，范围比本次改动大，已单独记录。钉子：`scripts/test-kernel-chunk-attach.mjs`（headless；Case 0 是"关掉修复"的修复前对照，Case 6/7 分别钉两类具名兜底，Case 8 钉载入器契约，Case 9 直接钉 `diffReplaceRegions`）与 `scripts/test-kernel-chunk-attach-ui.mjs`（`test:kernel-chunk-attach-ui`，LF + CRLF；非空洞性已用"把 `onChunksApplied` 关掉重新构建"验证——那时套件如期以 `attach-unmappable` 失败）。

**性能（计划六 Task 4，5 项安全优化已全部落地，2026-08-21）**：`.superpowers/kernel-performance-assessment.md` 是 2026-08-17 的纯测量基线：真实 app 里单次按键在 100 KB 文档上阻塞主线程约 **450–530 ms**，成本主要是每次提交 2 次（结构操作 4–6 次）全文重新 parse + 全文字符映射重建。评估给出 5 项"不削弱任何证明"的安全优化与 1 项会用"编辑是局部的"假设替换证明的增量映射方案（**后者依然明确不做**——与本程序历史上多次字节保真事故同形）。**5 项安全优化现已全部落地**（分支 `perf/kernel-large-doc`，各自带 headless 行为钉 + 全量内核门禁）：① 内核模式跳过 `markdownUpdated` 序列化（`5d35a87`）；② `applyKernelTransaction` 复用刚证明过的投影图（`bindMap` 的 `pmDoc.eq(parsed)` 收养守卫，PERF-1 钉）；③ 内核 parse 按精确字符串做 LRU memo（`syntax-index.js`，index 与 raw 两个缓存**绝不共享**——`injectHighlightNodes` 变异树）；④ charMap 惰性按块构建（非空文本块的三重证明推迟到首次访问；`pairForContentPos`/`rawToPmPos` 用 content.size / mdast span 预过滤不强制物化；只读计数徽章的全图扫描挪到 150 ms 尾随防抖）；⑤ 健康路径 verify 解析 200 ms 防抖（**同步保留三处**：gateway `rewrote` 的改写型提交（治愈/种子消解/前缀）、占位符会话结束（且会话中**绝不**触发——Case PERF-3 的回归教训，UI 套件抓到）、rebind 失败时的修复路径；flush 读者强制先跑）。**A/B 实测**（分支自报——同机同 100 KB 语料同方法；集成评审判定与阶段表算术自洽，但无原始探针日志、未独立复测）：单键同步阻塞 ~257 → **~130 ms**（−49%），12 键连发总阻塞 2.56 → 1.20 s，单任务峰值 261 → 139 ms；防抖任务（verify+状态扫描，~106 ms）在连发结束 200 ms 后空闲执行。**默认启用仍不够格**：130 ms 仍远超 16 ms 帧预算——只有增量映射方案能达标，而它以证明换速度。（原文此处还写着"`CHUNK_THRESHOLD`（120 000 字符）以上仍无法 attach"——该限制已于 2026-08-21 解除，见上方「2026-08-21 超阈值附着」段。）**集成评审三条件已闭（2026-08-21 合并时）**：① memo 变异金丝雀——两个缓存共享对象、契约是"消费者不得 mutate"，测试态 `setKernelMemoTestFreeze` 深冻结每个入缓存条目（mode-headless 与组合矩阵全程开启），任何未来消费者变异直接抛 TypeError，canary 钉在 `test-source-kernel-index.mjs`；② 强制 verify 名单逐项钉死——`flushMarkdownSettled` / `getRecoveryMarkdown` 各自的 pending-verify 强制运行 + 结构操作 subsumption 微用例（防抖 verify 只读 LIVE 状态、绝不套用陈旧 diff）钉在 mode-headless Case PERF-2b（blur 项按设计说理排除：blur 不读字节、计时器照常触发）；③ 确定性调用次数上界（Case PERF-4，独有拼写字符串保证与 LRU 历史无关、无 sleep）：attach ≤1 次内核 parse 且 0 次 charMap 构建、单键 ≤1 次 parse 且 ≤2 次 charMap 构建、结构 Enter ≤1 次 parse。三组钉子均做过非空洞验证（逐一移除被守卫的性质、确认对应断言失败后还原）。
...
- **斜杠菜单块类型/块插入域**（计划四之后新扩展的域，不属于计划六）：`/h1`–`/h6`、`/ul`（bullet）、`/ol`（ordered）现在把查询文字原子替换成对应的标记前缀，作为一次内核事务提交；`/table`、`/code`（含 `/js`/`/python`/`/mermaid` 等语言变体）、`/math` 现在把查询文字原子替换成新建的空块并把光标放在可以立刻打字的位置。`/mermaid` 是随 `READONLY_CODE_LANGUAGES` 删除自动解锁的（不需要单独接线）；`/math` 需要块级公式本身先变为可编辑才有意义（否则新建的块里没有可插入光标的位置）。`/image`、`/divider`、`/task`、`/text` 在该批次仍不路由（当时的理由：图片块和分隔线是没有文字位置的 PM 叶子，裸 `- [ ] ` 对 remark-gfm 不构成任务项，完全空的顶层段落没有 Markdown 字节可写）——**四项已于 2026-08-20 全部接通**，见下方「2026-08-20 斜杠菜单收官」段（`/text` 当时仍限文档末尾，该限制 2026-08-21 解除）。`/quote` 不属于该批次，因为它**更早**（2026-08-19，`123100f`）就已修复并正常提交——本文旧版在此处写的「永远不会被解锁」与事实相反，勘误见上方 5.2d 该项的说明（UI 回归 `test:kernel-quote-ui` 自 `b106f95` 起钉成功场景）。**2026-08-22（「引用内嵌套」报告）：块类型转换域对引用解锁**——`/h1`–`/h6`、`/ul`、`/ol` 现在也可用于祖先链为**纯 blockquote**（含多级 `> >`）的查询段落/标题：`block-type.js` 的 `quoteChainNodeAt` 只沿 blockquote 下钻（链上出现列表项即停、照旧拒绝），嵌套路径在返回事务前多跑一个两轴重解析证明（`provenQuoteConversion`：候选在同一 start、同一引用深度处解析出目标结构且无内容 + `outsideSignature` 切除区外逐字节存活——该副本对"包含区域的祖先"继续下钻子节点，否则引用内兄弟段落不受签名保护）；顶层路径保持纯位置论证、字节不变。控制器 `runBlockTypeFromQuery` 无改动（`requireMap: true` 原样成为引用内空标题/空列表项可配对、可落光标的前置证明）。打字路径（`# `/`- `/`1. ` 标记补全空格）在引用内**本就可用**（`spellMarkerCompletingSpace` 的证明与深度无关），2026-08-22 起在 marker-space 套件补了引用/二级引用 pin。同日第二批：**块插入域的 caret-INSIDE 目标也对引用解锁**——`/table`、`/code`(+语言)、`/math`、`/task` 在纯引用链内可用：拼写 = 顶层拼写 + 每条**续行**逐字节携带查询行自己的引用前缀（code-map 对每条内容行要求完整前缀，所以引用内空内容行是 `'> '` 而非裸 `'>'`），锚点经同一变换重映射（一个被探明的不对称：code/math 的列 0 锚点留在**行首**——引用 code map 把该行唯一单元锚定在含前缀的行首，前缀后的偏移解析不到任何位置）；两轴证明沿引用链运行（`outsideSignatureThroughContainers`）。顺带修了一个先前就存在的洞：**带空前缀内容行的引用围栏**（`'> ```js' LE '> ' LE '> ```'`，斜杠 /code 在引用内写出的正是这个形状，源码模式手写也能达到）首次输入无可证明拼写（'open' 解码出幽灵换行，'fill' 把字符落在既有前缀**之前**成 `'> x> '`）——`empty-code-insert.js` 新增第三拼写 **'fill-after-prefix'**（仅当锚点字节恰为前缀时尝试，同样过完整证明）。仍拒绝的残余：`/divider`、`/image`、`/text` 在引用内（caret-after/占位机制未在引用前缀下证明）；引用内 task 贴着引用内列表的合并形状（合并读法只在根级论证）；链上有列表项的一切形状；`/quote` 再嵌套。锁定：`test:source-kernel`（blocktype 第 10 节、blockinsert 第 13 节、marker-space 引用行、empty-code Case 6b、task-seed 引用拒绝）、`test-kernel-mode-headless` Case B6/B7 + IQ1-IQ3、`test:kernel-blocktype-ui` §4.5 与 `test:kernel-blockinsert-ui` §2d（真实应用,LF+CRLF）。
...
- **`/task`**：一次内核事务写入 `- [ ] ` + **U+00A0 种子**。这是"还没有正文的任务项"唯一可表示的拼写——实测每种纯 ASCII 拼写（`- [ ] `、双空格、Tab、`- [x] `、`* [ ] `）对 remark-gfm 都是 `checked: null` 的普通列表文字；`shapeAgrees` 要求重解析结果恰为一个 `checked === false` 的 bullet 项、唯一内容恰为该种子字节。光标落在种子之后。种子在会话台账里记为**第三种空白 provenance**（`ascii: ''`，"不代表任何按键"，`markdown-document.js` `acceptWhitespaceMarks`；空白自愈要求非空 `ascii`、消解要求恰为 `''`、无台账的 U+00A0 谁也不认领——三种 provenance 干净分片）。第一个正文字符落下时由 `commands/task-seed.js` 在**同一笔编辑**里删种子+插正文（`spellTaskSeedInsert`：`blockEditIsObservable` 复用 + 父级 `checked` 不变的额外证明，插入与首键之间的勾选也能存活）；作者自带（重开文件后台账为空）或空白自愈写下的 U+00A0 **永不被消解**。**与 legacy 的行为差（改进，教程已写明）**：插入后立即保存，磁盘上是真实 `checked:false` 任务、冷重开复选框仍在；legacy 在保存等耐久边界把空任务降级为 `- [ ]` 字面文字（`editor-api.js` `flushMarkdown` → `demoteEmptyTaskItemsInView`）。为此 legacy 的空任务**降级分支在内核模式被关闭**（`createTaskListInputPlugin({ kernelMode })`，`editor-task-list.js`——它的 lone-NBSP 迁移条款曾以 `appendTransaction` 骑在 projection 批上**绕过 gateway**，把字节完好的新复选框改写成 `[ ]` 字面文字；marker 转换不动，legacy 默认行为逐字节不变。分类器本身的这个洞——projection 批上的 appendTransaction 旁路——仍开放，见 §7 待办）。保存-未打正文-重开后种子成为作者字节，继续输入接在其后而非消解（fail-closed 方向，headless 钉住）。**位置性拒绝——2026-08-21 已取消（可预测的列表合并）**：紧邻既有同记号列表上/下方（或夹在两者之间）的段落里 `/task` 此前拒绝（空行只让 CommonMark 列表变松散、不结束它，插入的项会与既有列表合并，结构恒等的轴 (a)/(b) 拒绝自己没写的合并）。现由 `commands/list-merge.js` `provePredictedListMerge` **预测并逐项核对**该合并后接受：先要求候选字节严格是 baseline 把 `[start,end)` 换成写入字节的 splice（结构比较因此只谈 parse，不谈调用方是否诚实），再要求合并后的 list 恰好横跨 `[上邻.start, 下邻.end + delta)`、其 items 依次是「上邻 items ++ 我们这一项 ++ 下邻 items」、每个邻居 item 的**完整子树签名**（type/span/checked/spread/ordered/start）按 delta 平移后逐字相等、我们这一项仍过 `taskItemAgrees`（真 `checked:false` + 内容恰为种子），最后在**加宽后的并集区间**上再跑一次 `outsideSignature`。**唯一被接受的差异是合并后 list 自身的 `spread`（紧凑→松散）**：mdast 里紧凑项与松散项结构完全相同（都是 `listItem > paragraph`），projection map 看不见这一位，拒绝它等于拒绝真实文档里 `/task` 最常见的位置（2026-08-21 real-doc campaign 在 `docs/handoff-mode-switch.md` 上 7 个候选段落全部紧邻列表、整步跳过）——判断与理由写在 `list-merge.js` 头部 ADR。**不同记号**（`-` 与 `*`、`-` 与 `1.`、`1.` 与 `1)`）CommonMark 本来就不合并，走结构恒等轴，一直可用。**仍拒绝**：邻居是**缩进**列表（它会被吞成新项的子列表——item 账目对不上，实测 18 种形态，`unsupported-structure`）。钉：task-seed 套件 §1c（含 spread 翻转与不同记号的前提断言）、blockinsert 套件 §13（证明本体的对抗单测：字节关系、缩进吞并、ordered 不符、区间不是根子节点）。原「补救=列表项尾 `Enter` 续项」仍然可用且已修好：**勘误（2026-08-21）：这条「补救」在写下时实际是坏的**——`splitListItem` 给任务项续项写的是裸 `- [ ] `，重解析降级为 `checked:null` + 字面 "[ ]" 文字、光标锚点无字符映射单元（`caret-unmappable:split-list-item`）、下一键落进**错误的项**（实测勾选/引用/嵌套/CRLF 各形态一致，仅"标签中间分裂"一格幸存）。2026-08-21 用户报告后修复：任务项分裂后**没有内容的那一侧**（项尾 Enter → 新项；标签起点 Enter → 原项，原项保留自己的勾选态）经候选重解析证明后写入**同一枚会话台账种子**（`ascii:''`，同一消解命令族，键盘/粘贴/IME 首字符同样消解；证明失败回退到旧字节，绝不新增拒绝）；勾选项的续项恒为 `[ ]` 未勾选；**会话内新建、从未输入正文的种子项上按 Enter 走 lift-out 退出列表**（与普通空项同一条 `exitEmptyListItem` 出口、同一 caret 平价缺陷；ledger-gated——重开文件后的 U+00A0 是作者字节，Enter 照常分裂、绝不删除，2026-08-20 审计钉仍成立）。钉：task-seed 套件 §6、headless Case I5g/I5h、`test:kernel-task-item-ui` Enter 段）；新种子上按 `Backspace` **剔除该项（2026-08-22 起，Typora 语义）**：会话台账种子项对 Backspace 与 Enter 同样"实际为空"——顶层走 `exitEmptyListItem` 退出列表、嵌套走 `outdentListItem` 先反缩进一级（`router.js` Backspace 分支 + `delete.js` `liftEmptyListItem`，同一 ledger-gated 门：重开文件的 U+00A0 是作者字节，仍走文本路径）；此前该键的唯一答案是下述空任务墙的拒绝——用户报告（截图=在列表里剔除空任务项被 toast 拦下）指出这是「字符级删除不可表示」误推广到「按键必须拒绝」，而结构化路由删的是整行、完全可表示。**墙本身仍在**，看守一切非按键路径的删除事务（剪切、插件、程序化 tr.delete）：吃掉台账种子且段落归空的删除提交后会降级为 `checked:null` + 字面 "[ ]"，仍以 `empty-task-unrepresentable` 拒绝——**这堵墙在 2026-08-20 对抗审计前并不存在，是审计后补建的**：此前 `syntax-index.js` 用 `String.trim()` 计算列表项 `empty`（trim 会剥掉 U+00A0），种子项被误判为空项，Backspace/Enter 走 `exitEmptyListItem` **无声删掉整行**（零 toast + `caret-unmappable` 诊断 + 保存后落盘无该项）；即便绕过结构路由，字符级删除也会提交 `- [ ] `（重解析为 `checked:null` + 字面 "[ ]" 文字）。修复=（1）`empty` 改为 ASCII-only 判空（与解析器一致：U+00A0 是内容）；（2）新命令 `taskSeedDeleteRefusal`（`commands/task-seed.js`）：吃掉台账种子且段落归空的删除，经候选重解析证明项会降级后，以专用码 `empty-task-unrepresentable` 拒绝（有自己的 toast 文案，注明出口=撤销/输入正文/回车退出列表——第三个出口是 2026-08-21 任务项 Enter 修复新增的）；单步与多步删除批都过这堵墙；无台账/自愈 provenance 的 U+00A0、以及留有其它正文的删除一概不认领。四层钉死（task-seed 套件 §5 钉未入账种子的 not-structural + §7 钉已入账种子的剔除出口、gateway TS6 钉墙本体、headless Case I5b 钉「墙看守事务 + 按键走结构出口」两个契约、UI 3c 步在真实四项列表上驱动剔除——原 1b 步的拒绝钉随 2026-08-22 契约翻转改写）。
...
- **`/text`**：**不写任何字节**——完全空的顶层段落没有 raw 表示，命令只删除块自己的字节，光标由 split-placeholder 机制安置。两条路由：**（1）查询块是最后一个根级子节点** → 删除它及其后全部空白尾字节（纯后缀删除，签名相等证明：候选的完整解析 = 基线解析去掉查询块、偏移不动）；光标：剩余文档以列表/表格/围栏/原子结尾（或已空）→ 尾部虚拟对（`requireMap`）；以段落/标题结尾 → 控制器经既有 fail-closed split-placeholder 会话物化一个 VOUCHED 占位（无分隔前缀提交是字节正确的，因为命令**证明**保留字节以空行结尾）。**（2）文档中段（2026-08-21 新增，收口用户报告「源码内核中「正文」目前只能用于文档最后一个块」）** → **最小删除**（`{from: blockStart, to: blockEnd, insert: ''}`，两侧分隔符一字不动），留下的空行游程与结构 Enter 在同一位置写出的**字节完全相同**（`甲\n\n/text\n\n乙\n` 删块 = `甲\n\n` + `\n\n乙\n`，与在 `甲` 尾按 Enter 的结果逐字节相等），因此走同一个 `pendingPlaceholder`**单对象**会话（`buildProjectionMap` 明确豁免 trailing floor 的那一支，Case 13 的形状）。控制器侧：**提交前**从活视图读查询块的**顶层索引**（`topLevelIndexAt`，纯 PM 侧事实，不做 mdast↔PM 索引配对假设），提交后复核「恰好少了一个顶层子节点」且锚点仍不可解析，再对前 `index` 个子节点的 nodeSize 求和得到落点——`materializePlaceholder` 自身 fail-closed（证明不了就把节点撤回、重新平绑）。**两条独立证明，互不覆盖**：(a) **删除证明**——`outsideSignature` 要求删除区域之外的一切（类型+区间，后段按 delta 归一）逐字相等，抓的是**列表合并**（`- a` / `/text` / `- b` 删中间块后两个列表并成一个松散列表，合并后的节点横跨区域被跳过 → 签名不等 → 拒绝）；(b) **收敛证明**——在锚点插入一个探针字符再解析，必须得到恰好覆盖该字符的**根级 paragraph**、其余部分不变，抓的是**惰性续行**（`甲\n# /text\n\n乙`：标题可打断段落，删除本身干净，但锚点上打字会被 `甲` 吸收成续行）。用构造证明而不是分隔符正则，因此**接受**正则会误拒的形状（下方紧跟标题/列表/围栏而无空行——它们本就能打断段落）。锚点 = 被删块自己的 start（删除后严格落在空行游程内部：顶层兄弟之间至少隔一个行尾，故下一块只会移到 `start+1` 或更后）。剩下的唯一拒绝是**具名**的 **`text-neighbors-would-merge`**（两条证明任一失败），文案给出补救（直接删掉斜杠文字、手动编辑该块）；容器内的块（引用/列表项里的段落不是根级子节点）与所有其它 slash-insert 目标一样走通用 `unsupported-structure`。**会话平价**（headless Case I8c 逐条比对）：虚拟对 `{raw, prefix:''}`、会话中保存写下折叠后的字节（重开无占位——不伪造任何东西）、在别处提交即结束会话并把孤儿占位 reconcile 掉、地图恢复——与 Enter 的 Case 13/14 完全一致。
...
- 钉子：`test-source-kernel-blockinsert.mjs` 命令小节 10–12b（12b = `/text` 文档中段：十余种落地形状 + 两条证明各自的 load-bearing 反例）+ `test-source-kernel-task-seed.mjs`（`test:source-kernel`）、`test-kernel-mode-headless.mjs` Cases I5–I8c + `test-kernel-trailing-atom-typing.mjs`（`test:kernel-headless`）、UI 的 `test:kernel-task-item-ui`（含"尴尬时刻保存 + 冷重开仍是复选框"闭环）与 `test:kernel-leaf-insert-ui`（LF+CRLF，两条具名拒绝文案逐字断言 + 冷重开），以及 **`test:kernel-real-doc-campaign-ui`**（2026-08-21 新增，见下段），均已入 `test:kernel-ui`；每个新测试都在修复前 scratch 构建上确认会失败。深度记录：`.superpowers/sdd/2026-08-17-source-kernel-default-on/` 的 `slash-completion-report.md`（机制核验与勘误）、`wf-task-item-report.md`、`wf-gateway-report.md`、`wf-leaf-items-report.md`、`peer-editor-research.md`（采纳与偏离已记录：`***` 回退是证明驱动而非同行惯例；gapcursor 按其自身"仅视图伴侣"结论保持未挂载）。

**2026-08-21 真实文档战役（`scripts/test-kernel-real-doc-campaign-ui.mjs`，npm `test:kernel-real-doc-campaign-ui`，已入 `test:kernel-ui`）**：用户明确要求「拿成熟文档复制一份，在很多位置插入/编辑」——其余每个内核 UI 套件用的都是为它自己写的十几行 fixture，正好对这类问题全盲。本套件把仓库里三份**真实文档**复制到 /tmp（原件只读不写）：`docs/custom-shortcuts-architecture.md`（4 表格 + 6 围栏 + 引用 + 两种列表，attach = `partial`——表格是只读叶，诚实结果）、`guide/basics/rich-and-source.md`（YAML frontmatter + 独立图片 + 行内 HTML + 行内公式，attach = `normal`）、`docs/handoff-mode-switch.md`（~10.5K，最深标题层级 + 4 围栏，attach = `normal`）。每份文档在内核模式下跑同一套战役：段落首/中/尾三处打字 → `/text` 文档中段 + 占位里打字 → `/h2` + 打字 → `/task` + 标签 + Enter 续项 + 第二标签 → `/table` + 首格打字 → 既有围栏内编辑 → **零编辑**的 rich↔source 往返 → 保存 → 冷重开。**目标由文档自己决定**：`plainParagraphs`（raw 字节即可见文字、全文唯一）给候选，`pickTarget` 用**纯命令层**试算哪一处能落地（只用来选位置，期望值一律另算），文档给不出的步骤明确 skip 并打印原因（handoff 文档的 `/task` 当时因为所有候选段落都紧邻列表而整步 skip——正是那条既有的合并拒绝；该拒绝已于 2026-08-21 由 `provePredictedListMerge` 取消，这一步现在能在该文档上真正跑起来）。**期望值全部是「上一次字节 + 本次编辑」的 splice**，绝不写死整篇文档。每步断言三件事：字节、零意外 toast、零 `projection-mismatch`（内核自己的视图↔重解析 diff 就是「静默分歧」探针）+ FATAL 诊断清零。已量化的**已知良性**只有两条，都写在文件头：`/text` 的 `caret-unmappable`（空行锚点本就没有 PM 位置，正是交给占位的那一步），以及**每次 `/task` 种子消解恰好一次 `projection-mismatch`**——这是本战役量出来再解释的：消解是不对称提交（PM 事务只插入标签，raw 编辑同时删掉 U+00A0 种子），所以视图有一拍是 `<种子><标签>`、重解析是 `<标签>`，修复 reconcile 正是种子从屏幕上消失的机制；预算旁边紧跟着「视图里必须已无 U+00A0」的断言，所以一个**没有修复**的 mismatch 仍然会红。历史上「含表格文档首次提交必有一次 mismatch」的容忍额度**没有沿用**：四表格文档跑完 15 步实测为 0，于是预算保持 0。首轮跑出的真实缺陷是**测试自身**的：单行拖选在真实文档的**折行段落**上只选中第一视觉行，斜杠菜单因此根本不开——已改为「点首字符 + Shift 点末字符」并把成功条件收紧为「选区文字 === 整块文字」。三份文档合计 40 步全绿。

**2026-08-21 CRLF 软换行加宽（进度账本「queued, next」项收口）**：`textUnits`（`lib/source-kernel/character-map.js`）此前把 CRLF 软换行的 `\r` 记成普通 width-1 `char` 单元、只把 `\n` 记 `linebreak`，而 ProseMirror 对**任何拼写**的行尾只持有一个 `'\n'` 字符——每个软换行让 `visibleLength` 比 `content.size` 多 1，投影图的尺寸证明失配、charMap 置 null，**CRLF 文档里所有带续行的段落/引用段落整块只读**（用户报告的日常形状：列表项续行，Tab 在那里答「只读」）。修复=**加宽**：一个行尾（`\n`、`\r\n`、孤立 `\r`）就是**一个** width-1 `linebreak` 单元，raw 跨下整个行尾加续行前缀（`consumeSoftBreak` 原逻辑），单元记录 value 拼写 `ending`；孤立 CR 的带前缀续行（旧模型直接 null）**首次**可映射。三处走查同步，缺一即全族拒绝：`highlight-syntax.js` `offsetTables` 按 `ending.length` 推进 value 索引（否则 CRLF 段落的高亮注入失效→两链分歧→该块继续只读）；`commands/link-toggle.js` `visibleTextOf` 把 text 值行尾归一为 `'\n'`（否则 index-compatibility 不变量拒绝所有 CRLF 软换行块的链接命令）；`test-source-kernel-inline-html.mjs`/`test-source-kernel-highlight-consistency.mjs` 的 PM 尺寸模拟器同样归一。**边界语义**：`\r\n` 对内偏移不再是任何可见边界（boundaries/startBoundaries 无该键），旧的 bisection 拒绝在散文图里不可达——字节层 `splitsCrlfPair` 仍在 `applySourceTransaction` 唯一写入点拒绝一切 raw 算术写入；**code map 不动**（CM 桥需要逐字节寻址，`bisectsLineEnding` 继续守护代码块）。组合矩阵 **D2 家族整族消失**（154 个组合收窄，`fix-scheduled` 契约兑现；旧被 D2 毯子分类遮住的 `> ` 字面续行 CRLF 形状现以 D3 具名打印）；`test-kernel-projection-map.mjs` 的 KNOWN LIMITATION 钉按其自述翻转为正向断言；`test-kernel-line-start-whitespace-ui.mjs` 的 CRLF 软换行只读分支删除，CRLF 跑与 LF 相同的 NBSP 重拼写断言。钉子：`test-source-kernel-charmap.mjs` CRLF 加宽小节（列表续行/引用/孤立 CR/LF 控制组，含 `ending` 字段）、`test-kernel-mode-headless.mjs` Case CRLF-SB（续行打字两位置 + 软换行删除恰好吞 `\r\n  `）、link-toggle CRLF 小节改为正向、`test:kernel-line-start-whitespace-ui` 双行尾。

**2026-08-21 裸标记第二出口（`*`/`-`/`1.`/`#` + 普通文字）与「选区也是源码权威」**：用户报告在内核模式空行输入 `*` 后继续正常打字「无法形成映射、构建报错」。实测（`甲一\n` 带尾随换行的 fixture，逐键真实按键）比拒绝更糟——是**静默错位**：`*` 落盘后重解析成空 bullet 项（字节优先，正确），但 (1) 纯文本提交路径的修复 reconcile（`verifyPlainTextProjection`）不带任何光标目标，`tr.replace` 的 diff 映射把光标抛进尾部占位段落；(2) 后续每一键都路由到错误的块——`*a` 打出 `甲一\n\n*\n\na`（死的空列表 + 被割裂的正文），零 toast，仅 `map-refresh-failed` + `projection-mismatch` 诊断；(3) 连 2026-08-20 已建成的**空格补全出口也一并失效**（`*` 后按空格拒绝 `unsupported-structure`）——marker-space UI 套件当时能过，是因为其 fixture 无尾随换行、占位块的 raw offset **恰好**与标记行行尾重合，巧合掩盖了光标丢失。此外裸列表标记的空项配对（`syntheticEmptyItemParagraph`）`mdBlock` 与 `charMap` 双 null、**没有任何字节锚点**，`markerRawOffsetAt` 对列表族根本解析不出 offset——空段落场景的 `- `/`* `/`1. ` 空格补全其实从未通过自己的路由成立过。三层修复，全部沿既有证明模式而非特例补丁：

- **光标随字节走**（`editor-kernel-mode.js` `verifyPlainTextProjection` + `editor-kernel-projection-map.js` 新 `rawToPmCaret`）：结构变化的修复 reconcile 现在把**已提交事务自己的 selection anchor**（字节真相）经 parsed 文档的投影图解析回 PM 位置，选区骑在**同一笔** reconcile 事务上（Task 11 Bug 3 的同一协议——Crepe Vue 列表项的 DOM 未挂载前，后补 selection dispatch 到不了）。`rawToPmCaret` 是比写路径弱一级的**安全**问句（「光标可以坐哪」）：先走 `rawToPmPos`，失败则找 raw span 含该 offset 的**空文本块**配对——空文本块只有一个光标位，这是推导不是猜测；写入方仍只用 fail-closed 的 `rawToPmPos`。
- **空项配对补字节身份**（`mdItem` 字段骑在 syntheticEmptyItemParagraph 配对上）：裸标记的 `markerRawOffsetAt` 由 `mdBlock.position.end ?? mdItem.contentStart` 推导——列表族从此与标题族同权,空格补全在空段落场景真正接通。
- **`spellMarkerFollowingText`**（`commands/marker-space.js`,与空格补全同文件同族）:裸标记中间态的第二出口。规则与第一出口对偶——**空格说「它是语法」,普通文字说「它是内容」**:`*` + `a` = 字面段落 `*a`,恰是字节 `*a` 本来的重解析结果。候选重解析证明**降级可见**(一个恰拼 `标记+字符` 的 text 节点从标记自身 offset 开始、原结构消失),不可证明一律 `not-structural` 回落原行为。`>` 刻意不认领(裸 `>` 已有 empty-quote 虚拟锚,一个字节不能有两个主人);一切空白(含 U+00A0)不认领(归补全/re-speller/台账族)。路由骑 `handleTextInput`,run-growth(`#`→`##`)优先,第七个 `#` 落进本命令成为字面文本(Typora 同)。`requireMap: true`(结果是普通段落,必须可证明可映射,否则一字节不写)。
...
**2026-08-22 嵌套兄弟 Tab + 会话空白任务标签（用户截图：任务列表里冒出字面 `• [ ]`、嵌套项尾 Tab 无反应）**。两个独立缺陷，一次修复：(1) **嵌套项的 Tab 缩进从未生效过**——`commands/indent.js` `previousSibling` 在每条候选行的**裸行首**探测（`lines[li].start`），而嵌套兄弟的行首是缩进字节，`listItemAt` 把它们解析到**外层项**（item.start 在缩进之后），深度不匹配 → 兄弟永远找不到 → 深度≥2 的 Tab 一律静默拒绝（此前所有 indent 套件的形状恰好全是顶层兄弟，从未踩到）。修复＝探测点越过该行自己的 `[ \t>]*` 前缀落在 marker 上；续行探到属主项，由既有 depth/listStart 过滤裁决。(2) **种子+拼写空格的任务项 Enter 无限繁殖种子兄弟**——Enter 播种续项、首键是空格时尾随机制正确拼成第二枚 `ascii:' '` 的 U+00A0，但 `isLedgeredSeedOnlyItem` 只认「恰好一枚种子」，该项从此判为有内容 → 再按 Enter 又分裂出新种子项，永无出口。修复＝谓词按**出处分区**推广并改名 `isLedgeredWhitespaceTaskItem`（task-seed.js）：内容全为不可见空白（U+00A0/空格/Tab）且**每枚 U+00A0 都有台账条目**（种子 `''` 或拼写空白的非空 ascii）→ 实际为空；重开文件（空台账）与半入账混合一概不认领，作者字节照旧分裂、绝不删除。Enter 在任意宽度都走整行退出；**Backspace 保持逐键粒度**——两字节标签先走文本路径删一枚（结果仍是可表示任务），仅剩**单字符**（其字符级删除无拼写、会降级勾选框）才路由整行出口——顺带治愈了 heal-出处单枚 U+00A0 的旧死端（此前被 veto 卡死，只能 Enter/撤销）。用户文件里已有的 `- [ ]   `（尾随 ASCII 空格）是**旧构建**落盘的字节，重载即降级为字面 `• [ ]`——今日构建已不产生该拼写（种子上打空格实测拼成 NBSP）；旧行是作者字节，按普通文本编辑删除即可，内核不做自动迁移。钉：`test-source-kernel-indent.mjs` 嵌套兄弟节（平文/任务/引用 + 首子项无兄弟负控）、`test-source-kernel-commands.mjs` 会话空白任务节（真实命令赚取台账 + Enter 退出 + Backspace not-structural + 冷文档/半入账两负控）、task-seed 套件两处钉随契约翻转改写（heal-单枚 Enter/Backspace 均改判退出，注明取代理由）、`test:kernel-task-whitespace-ui`（真实应用整剧本：Enter 播种→Space 拼 NBSP→Tab 嵌套到深度 3→Enter 退出→落盘无降级拼写，已入 `test:kernel-ui`）。

**2026-08-23 清空删除的单行豁免缺口（用户截图：引用内嵌套项 Backspace 后光标传送到下一个引用）**。`editsClearBlockLine` 刻意放过单行块（「清空段落=普通空块，不重构文档」），但该论断对**列表项自己的单行段落**为假：裸空的 marker 行是上一兄弟的 **setext 下划线**（`- 你是谁\n  - ` 解析为 listItem > h2「你是谁」，根级与引用内一致实测），任务的 `- [ ] ` 则把勾选框降级为字面文字。删除未经证明提交 → map-refresh 对重构后的解析失败 → 区域修复重写视图 → `caret-unmappable` 回退把光标丢在 reconcile 落点（下一个引用）——完整传送链的三个诊断事件逐一复现。**任务标签清空降级还是活体路径**（此前归因「旧构建残留」不完全：删掉任务标签最后一个字符当场写出 `- [ ] ` 并降级——用户文件里的 `- [ ]   ` 行即由此而来）。修复＝`spellEmptyListItemDelete`（content-delete.js，gateway 单步删除在种子墙之后接线）：认领「一次纯删除清空列表项自己的单行段落（项无续行/子列表）」，三向作答——裸候选重解析证明是真空项（顶层 `- ` 等）→ not-structural 保持字面字节最小提交；证明陷阱 → 改写为删除+**台账种子**（`ascii:''`，与 indent 救援/`/task` 同一可表示拼写、同一溶解管道、同一 visually-empty 出口），`checked` 态保持，`respelledVisibleDelta = 1 - visibleLength` 走 rewrote-verify 修视图（与空白 heal 同构）；皆不可证 → 具名拒绝。幸存 U+00A0 是解析器内容 → 不算清空不认领（作者字节学说完好：命令只在用户自己的删除吃掉最后一个可见字符时运行）。批处理路径（proveBatchDelete）同一认领但**只证裸候选、无改写通道**：陷阱形状拒绝而非提交。gateway 套件 TS6 旁的旧钉（「非种子 U+00A0 删除保持既有字面行为=写出 `- [ ] `」）按新契约翻转——那个「既有行为」本身就是损坏；注明理由。钉：content-delete 套件新节（嵌套平文/引用/任务三救援 + 顶层裸字节负控 + 幸存内容负控 + 子列表负控 + 批处理拒绝）、`test:kernel-empty-delete-ui`（真实应用完整剧本：引用内 Backspace 清空 → 光标留在种子项、不传送 → 打字原位溶解 → 任务清空勾选框存活 → 落盘零降级拼写，已入 `test:kernel-ui`）。相邻回归 task-item/task-whitespace/trailing-backspace/kernel-mode UI + 全部无头套件复跑全绿。

**2026-08-23（第二批）文档中段空白游程 Enter + 删空段落的占位符归宿（用户报告：空白正文按回车被 `unsupported-structure` 拦截，「直接将上一个作为空行不行吗」）**。三层联动修复：(1) **splitTextBlock 的 GAP 分支中段化**（enter.js）：Enter 落在空白行（占位符段落的锚点）时把游程再延一行——历史上中段 GAP 刻意 fail-closed（当年 split-placeholder 只会在文档末尾物化），/text 中段化（2026-08-21）后该顾虑不复存在；每个拼写过 `gapInsertPreservesStructure` 重解析证明（除插入点后偏移平移外全树 type/span/叶值不变），裸 ending 在引用内会把 blockquote 劈成两半、由证明当场拒绝，改试 `ending+该行引用前缀`。旧钉「中段 GAP 必须拒绝」×2 按新契约翻转。会话扩链（`extendTrailingPlaceholder`——名字里的 trailing 已不准确，机制本身通用）内部正是经 routeStructuralKey→splitTextBlock 派生字节，此分支即其中段成功的根源。(2) **投影图链地板放宽**（editor-kernel-projection-map.js）：`pendingPlaceholders` 复数链原要求全部 voucher 在最后一个顶层块之后（尾链专用），重述为其真正保护的不变量——**voucher 不得落在任何叶块 span 内**；尾部、根级中段、引用内空白行（在 quote 容器 span 内但不在任何叶内）全部合法，落在真实内容内仍整图拒绝。Case 15b 翻转（顺带发现其旧断言失败时对巨型 map 对象的序列化会让套件假死超时——症状是 exit 137 无输出，勿再误判为 OOM）。(3) **删空根级/引用段落 → 占位符 voucher**（content-delete.js `spellEmptyListItemDelete` 新 placeholder 条款 + gateway `emptiedBlock` + controller bindMap(pending)）：段落无 marker、无可表示的空拼写，删除最后一个字符后幸存的空 PM 段落以 voucher 入会话（与 Enter 开的同一 split-placeholder 会话：打字填充、Enter 扩链、其它提交清退）；此前未认领时 map-refresh-failed → 修复删掉光标下的段落 → 下一键被吞（实测）。`provenParagraphVanishes` 证明该段落叶消失且其余节点逐字存活（偏移平移归一）。填充提交结束会话时的 `map-refresh-failed`+`projection-mismatch` 一对是**既设计的清理路径**（孤儿占位符由 `hadPlaceholders` 分支同步 verify 移除，无吞键窗口；空行折叠为段距与 Typora 重渲染一致）——诊断里见到这一对不必再查。钉：commands 套件中段 GAP/引用 GAP 两用例（旧拒绝钉翻转）、projection-map Case 15b（中段链成图 + 叶内 voucher 仍拒）、content-delete (h) 节（根/引用段落 placeholder 条款 + 部分删除负控）、`test:kernel-gap-enter-ui`（真实应用：中段 Enter×3 无拒绝 → 填字 → 删空 → Enter → 再填字 → 引用内 Enter×2 前缀延行 → 落盘逐字核对，已入 `test:kernel-ui`）。全部无头 + kernel-mode/empty-delete/trailing-backspace UI 复跑全绿。

**2026-08-23（第三批）占位符空段上的 Backspace/Delete（用户截图：大引用内 Enter 出的空占位段按 Backspace 被 `unsupported-input-type` 拒绝）**。根因：`commitTrailingPlaceholderEdge` 只认**文档末尾**的占位符（doc-end 硬门），第二批让占位符可以出现在文档中段/引用内之后，这两个键在中段占位符上落进 PM 的 `joinBackward`——跨父 `ReplaceStep`（诊断形状 `ReplaceStep[220,222]@paragraph:d2/cross-parent open0/0 <empty>`）网关只能匿名拒绝。修复＝gap-Enter 的严格逆操作：voucher 在物化时记录**本次 Enter 写入的 raw 区间**（`writtenFrom/To`——`ensureSplitPlaceholder` 记完整分裂拼写如 `\n>\n> `，`extendTrailingPlaceholder` 记单个 ending+前缀），控制器新增 `shrinkSplitPlaceholder`（镜像 extend 的原子推进/整体回滚），Backspace/Delete 且 caret 在**链尾**占位符时认领：弹出链尾、按记录区间整段删除＝字节级精确还原 Enter 之前的文档（Enter×3 后 Backspace×3 逐段对称退回）；无区间的 voucher（emptied-paragraph/exit 锚点骑既有行）走一行收缩（删上一行终止符+本行前缀）。纯命令 `shrinkBlankRun`（enter.js）双预筛（caret 须在空白行前缀末尾、被删字节只允许 `[>\t \r\n]`）+ `gapRemovalPreservesStructure` 重解析证明（与插入证明同一签名函数,负向平移；删掉唯一分隔行会并块/惰性续行，当场拒绝）。文档末尾的既有行为**不变**（trailing edge 先认领，一按回收全部尾部空行）；不可证一律具名拒绝并吞键，绝不再落到 joinBackward。钉：`test-source-kernel-commands.mjs` shrinkBlankRun 节（span 逆/行收缩/引用/CRLF 整对/并块拒绝/内容区间拒绝）、`test-kernel-mode-headless.mjs` Case 17b–17e（中段还原+undo/redo、链对称退回、引用形状、emptied 无区间回退；17c 特意中段——文档尾是 trailing edge 的既有钉）、`test:kernel-placeholder-backspace-ui`（LF+CRLF 真实按键：root+引用 Enter→Backspace、链退回、Backspace 后即打字、保存字节与 fixture 逐字相等，已入 `test:kernel-ui`）。非空洞性：修复前构建下该 UI 钉在第一个 Backspace 处失败；用户原文件的原手势复跑零拒绝零诊断。相邻回归 gap-enter/empty-delete/trailing-backspace/quote/kernel-mode/blockinsert/empty-fence UI + 全部无头套件复跑全绿。

**2026-08-23（第四批）引用中段的斜杠项:remark 列表 span 簿记的两张脸（用户报告「引用没有办法接续列表」——大引用尾段后 Enter 出占位行、输入 `/`、点「待办列表」被 `unsupported-structure` 拒绝）**。实测根因都是同一个 remark 行为:**引用内 list 节点的 `position.end` 会吞掉其后的空引用行,且吞多远取决于下一个兄弟是什么**(根级 list end==item end;引用内后随段落时停在空行后,后随列表时一直延到新 marker 前——字节未动,仅 span 簿记变化)。两处受害:(1) `insertBlockFromQuery` 轴 (a) 的 `end === insertedEnd`「无吸收」检查——引用内 `/task` 只要后面还有任何引用行(裸 `>`、`> `、内容)就恒拒,只有查询行是引用最后一行才成功。修复=`provenSpanEnd`:end 超出时要求**每个有位置后代都止于写入字节末尾**且 span 尾部字节全为 `[>\t \r\n]`(惰性续行由后代检查拒绝、同记号并入由 shapeAgrees 单项检查拒绝,各有负控)。(2) 两份 `outsideSignatureThroughContainers`(block-type/block-insert)的轴 (b)——在**引用内既有列表之后**转换/插入列表(`> 1. 甲` 后 `/ul`、`/ol`、`/task`)时,候选里前面那个未动列表的 end 被 remark 改记(15→18),端到端签名误判为语义变化 → 拒绝(组合套件抓到:菜单项静默无效,`/ul` 查询残留)。修复=`clampedNodeEnd`:容器 end 到最后一个有位置后代 end 之间若全为前缀/空白字节,则按后代 end 参与重叠判定与签名(两侧对称;真实吸收体现在后代行,照旧失配)。顺带查明两件非缺陷:块尾空格连打的 `projection-mismatch` 是尾随空格 NBSP 机制的 rewrote-verify 自愈(根级与引用内同现,字节正确,记入已知诊断);`/text`/`divider`/`image` 在引用内维持记录在案的具名拒绝。钉:blocktype 纯套件 §10(h)(i)(quote-mid 各转换+列表后转换,LF/CRLF)、blockinsert 纯套件 §13 d2-d4(quote-mid/嵌套 /task+惰性续行与同记号合并负控+列表后 /task+/table//js//math)、`test:kernel-blockinsert-ui` 2e 步(引用中段 /task+label 落盘)、**新** `test:kernel-slash-combo-ui`(LF+CRLF 组合矩阵:每个可用项在引用中段插入→CJK+数字+空格键入→代码块字面 Tab/表格 Tab 导航→Backspace 删除→字节精确保存→冷重开重投影,已入 `test:kernel-ui`)。全部无头+相邻 UI(blocktype/blockinsert/task-item/task-whitespace/quote/kernel-mode/stage3/placeholder-backspace/gap-enter/empty-delete)复跑全绿。

**2026-08-23（第五批）三个删除/分裂手势（用户连报三图）**。(B·字节损坏,最重) **标记尾分裂撕开闭合定界符**:PM 光标在行内标记末字符后的 raw 边界落在闭合定界符**之内**,`splitListItem/splitTextBlock` 在语法里下刀——实测 `- \`npm run test:source-map\`` 尾部 Enter 写出 `…source-map\n- \``,旧项行内代码静默失去闭合、新项成了字面反引号(用户截图的「空代码框删不掉」即此物;其上 Backspace 被拒是正确的 fail-closed,根因在 Enter)。修复=`snapPastMarkTail`(enter.js,两个分裂共用):块自身 charMap 为预言机,offset 之后到块末尾若**零可见字符**(gap 无单元,纯语法字节),分裂吸到块末尾;有可见内容跟随则原样中段分裂(负控钉住)。(C) **根级列表退出后正文被惰性吸收**:退出清出的空行锚点上,占位符 `insertPrefix` 为 `''`,而锚点前一行是内容行——提交即 `- 11312312\n2313` 单项吸收(引用侧 e1c00bf 早修,根级孪生漏掉)。修复=`placeholderForUnmappableAnchor` 前一行含内容时 prefix=ending(空白行上方仍 `''`)。(A) **段首 Backspace 并入 ATX 标题**:`joinParagraphBackward` 只认段落←段落,前块为标题一律拒(旧钉如此;用户要求 Typora 行为)。扩展=前块为**单行 ATX** 且本段**单行**时删除整个块间隙直接并行(`###### 3412321`+`121312`→`###### 3412321121312`),重解析证明(同深度标题、end 平移、区域外逐节点存活);多行段落、setext 照旧具名拒绝,旧拒绝钉按新契约翻转。钉:commands 套件 MARK-TAIL SNAP 节(代码/加粗/CRLF/中段负控)、indent 套件 join 节翻转+H6/引用/CRLF/多行+setext 负控、`test-kernel-mode-headless` Case 17f(退出+打字=分离段落,吸收拼写注册为 fixture 供回归时字节断言失败而非解析抛错)、**新** `test:kernel-join-split-exit-ui`(LF+CRLF 一条剧本串三手势:并入标题→标记尾分裂完整性→文末退出+两段正文,保存字节逐字相等+诊断无 unclassified,已入 `test:kernel-ui`)。全部无头+十一组相邻 UI 复跑全绿。

**2026-08-23（第六批）同行嵌套 item 记录错乱 + 渲染一致性套件（用户报告「自动衍生出 3.4 的多级标题」+ 指令「富文本渲染效果需要测试和验证」）**。先厘清:`3. 4.` 单行源码按 CommonMark 就是「项 3 含嵌套空有序项 4」(VSCode 同读),富文本如此渲染是**忠实投影**,非缺陷。真缺陷在排查中抓到:**`buildItem`(syntax-index)从行首解析 marker**,同行嵌套项(`1. 2.` 的内层 `2.`)拿到的是**外层** marker 的几何(marker `'1.'`、contentStart==自身 start),链式击穿——`rawToPmCaret` 的空块 span 变成 `[start,start]` 解析不到锚点 → `.` 提交后的 mismatch 修复丢光标(`caret-unmappable`)→ 下一键 `markerRawOffsetAt` 拿错 offset → `spellMarkerFollowingText` 拒绝 → 字符落进错块。实测:逐键输入 `2.3121312` 把文档打散成 `1. 2.` + 独立段 `3121312`/`2131`(用户的 3./4. 即此族残骸)。修复=buildItem 在「行首 marker 几何 ≠ 本节点 start」时**按节点自身 start 重推导** marker/spacing/markerEnd(indent 记 `''`——它的"缩进"是祖先 marker 而非空白,缩进族命令的证明自会拒绝拼不出的操作);常规项走原公式字节不变。修后逐键 burst 全程一体:`1. 2.3121312`→`2. 2131`→空项回车退出。钉:`test-source-kernel-index.mjs` 同行嵌套节(`1. 2.` 内层记录五字段 + `- 1. 甲` 变体 + 外层记录不变)、**新** `test:kernel-render-consistency-ui`(LF+CRLF,用户指令的落地:全块族语料〔h1/h6、四种行内 mark、嵌套引用、嵌套+同行嵌套列表、任务嵌套、表格、代码、hr〕在真实 app 渲染,DOM 走查归一为骨架串与 CommonMark 读法**逐块相等**,源码切换必须逐字节等于 fixture〔渲染不许改写字节〕,再加逐键 marker-burst 落盘钉,已入 `test:kernel-ui`)。全部无头 + marker/indent/task/list/quote/join-split-exit/kernel-mode 九组 UI 复跑全绿。

**2026-08-23（第六批补充）「表头 Tab 并行」复核 + 回环套件**。用户在旧进程(13:45 启动、载有 0.14.2 asar,早于 buildItem 修复的 14:16 安装)复测报「依旧存在」;在 v0.14.3 构建上逐一复测:项头 Tab 缩进字节正确(`   1. 2131` 嵌入项 1、marker rescue 改号)且视图与字节一致,不再拼进 `4.` 行;列表后隔空行段落的行首 Tab 由行首空白拼写机制落安全空白、无吸收。**手测前必须确认进程来自新 asar**(§1 既有规矩,此次真实踩中)。按用户指令新增 **`test:kernel-gesture-loop-ui`**(LF+CRLF 回环:同一文档两轮 编辑→保存→冷重开——第一轮 marker burst(`2.` 中间态降级)+同行嵌套裸 `4.`+项头 Tab 嵌套,第二轮 嵌套项续打+段落并入标题;每次保存字节逐字断言、每次冷重开渲染骨架逐块断言,已入 `test:kernel-ui`)。

**2026-08-23（第七批）组合测试扩充（用户指令「继续检测测试,各种组合测试,完善整个测试套件」）**。三件套:(1) **无头组合矩阵元素表 +7**(`test:kernel-combination-matrix`,40→47 元素、3206→4422 组合):图片 TITLE 槽 `![替代](img.png "题注")`(caption 的字节家)、链接图片(marked atom 邻域)、脚注引用+定义(第四种可打字行内原子)、对齐表格(table-ops 对齐行字节)、H1(降级手势的主语)、**块级 HTML**(唯一新只读形状——1 内核原子 vs PM 逐字符 run,fail-closed,立为新家族 **D4**〔known-degraded,分类机械限定在含 `html-block` 的组合的 paragraph 条目〕)、bullet 续行下嵌套有序列表(同行嵌套记录族);containment 增引用内有序列表/带题图/对齐表格,triple 增 `quote>list+para`/`quote>list+list`(第四批 provenSpanEnd/clampedNodeEnd 形状)与对齐表×mark 格。快照重基线 **+208 条、0 删 0 改**(键级 diff 验证纯增量;184×D4、24×D1 新元素毗邻 `table-br`),除块级 HTML 外**所有新构造在全部组合中完全可编辑**。(2) **`test:mode-switch-combination-ui` fixture 同步扩容**(对齐表、脚注、带题图、块级 HTML、引用内列表+尾段):harness 与真实 app 一致报 **2** 只读块(D1 `<br>` 格 + D4 HTML 段落——D4 首次在真实 app 中被担保计数),legacy 会话对新构造亦字节稳定。(3) **新 `test:kernel-feature-loop-ui`**(LF+CRLF 双循环 编辑→保存→冷重开,gesture-loop 的姊妹件,覆盖斜杠轮之后的手势族:第一轮 H2→H1 降级+对齐表体格打字〔`:---`/`---:` 分隔行一字节不动〕+引用内列表后尾段打字,第二轮 H1→段落降级+引用内列表项打字;每次保存字节逐字断言、冷重开骨架断言〔含 dl 脚注定义/表格 div 包裹的真实 DOM 形态〕,已入 `test:kernel-ui`)。无头门禁全量复跑绿。

**2026-08-23(第七批补充)真实输入探索会话(用户指令「使用 computeruser 等进行真实测试输入报错这类」)**。四场 CDP 真实鼠标/键盘会话(构建后 app、内核默认、逐字输入+截图目视+console/异常/对话框/诊断/toast 全程采集),全程 **0 renderer 异常、0 console error、0 意外对话框、0 字节分歧**(每场 disk==source view)。逐项verified:嵌套列表 Tab、`/table` 插入+表头打字+Tab 跨格、对齐表体格打字、引用内列表尾段、任务项**复选框真实点击**(`span.milkdown-icon.label`,`[ ]`→`[x]` 字节精确、冷重开渲染勾选)、H2→H1 降级、代码块内 Enter+字面 Tab、emoji/组合重音/全角标点逐字落字节、往返切换后续打、保存冷重开。两个**值得记录的发现**:(1) 逐字输入 `<script>alert(1)</script>` 到 `<script` 一刻段落成行内 HTML 承载块 → 整段只读(D4 姿态的真实输入面;后续击键全部具名 toast 拒绝、无 XSS 执行、无损坏——fail-closed 正确但"打字打到一半段落锁死"是真实摩擦);(2) **行内 mark 的定界符输入规则在内核被否决吞键**——逐字打 `*斜*与**粗**与\`码\`与~~删~~与==高==完` 落盘为 `*斜与粗与\`码与~~删与==高=完`,9 个触发 emphasis/strong/code/strike/highlight 输入规则的定界符击键各产生一条 `unclassified-transaction` 后被整键吞掉(PM 与字节一致、无损坏,但 Typora 习惯的 `**粗**` 手打路径**不可用**,只有工具条/快捷键 toggle 可用)。此前 veto 清单只记了列表 `- `/`1. ` 与 fence 的输入规则处理,mark 输入规则的吞键**未见记录**——列入待办:或证明为字节改写(定界符本就是用户敲的字面字节,加上规则删除的定界符与 mark 应用,可走 mark-map 的既有拼写),或至少具名拒绝并保留字面字符。测试驱动铁律补充:Crepe 任务复选框无 `input` 元素,点 `li > .label-wrapper .milkdown-icon` 才是真实手势。

**2026-08-23(第七批收尾)Computer Use「多级列表拼接」复核 + outdent 改号落地(用户指令「检查是否可修复」,证据 `docs/computer-use-input-verification-2026-08-23.md`)**。Computer Use 记录的 `2. 3.two / 3. 4.nested / 4. 5.out` 编号拼接在两条忠实键盘通道(insertText+结构键 keydown;完整 `keyDown` 带字符——与 CGEvent 等价)× 内核/legacy 四组复刻中**均不可复现**,结合该记录自archived的驱动错焦证据,降级为**疑似驱动伪影**(与 8-22 验尸同构;最终裁决待 Computer Use 驱动修复后重测)。复核发现的真实缺陷已修:**outdent 有序项编号不续**(`Shift+Tab` 后嵌套项自己的 `1.`/`2.` 原样回根级——CommonMark 语义正确、Typora 拼写错)。修复=`outdentListItem` 的 **ORDERED-MARKER RENUMBER**(indent 侧 rescue 的镜像:有序项落到有序父项下续 `parent.number+1`、用父列表定界符;bullet 父项/编号已对时字节不动;同一 `provenNestingOnly` 重解析证明把关,被拒时退回纯剥缩进,改号绝不使 outdent 失败)。**勘误(2026-08-26)**:本条当时把改号写成「CommonMark 语义正确、只是 Typora 拼写错」,并据此当作零渲染后果的纯拼写手势——次日 `68a0b38` 的 `sourceOrdinalPlugin` 让编辑器改显**作者字节里的号**(实测字节 `1. a / 5. b / 9. c` 显示为 1、5、9),该前提当场作废而无人察觉,两者共存约 17 小时。裁决(2026-08-26,用户):**改号保留**,`1. 甲 / 2. 乙 / ␣␣␣1. 丙` 上 `Shift+Tab` 必须读作 1、2、3;行为不变,要改的是理由——改号需证明的是**爆炸半径**(改写项须为有序列表**非首项**,否则数字变成列表 `start`、所有兄弟项一起改号),由 `commands/indent.js` 新导出的 `provenNonFirstOrdinal`(原名 `provenIgnoredOrdinal`)与 `provenNestingOnly` **两者都过**才放行。该谓词此前在命令+两份测试里三处独立复制,正是这份复制让死前提躲过审视;现两份测试改为 import,而 statemachine 的不变式拆成互不包含的两半(结构半边只看字节:至多一行超出前导前缀变化、marker 之后逐字节相同、行数不变;爆炸半径半边=import 的重解析证明),两半均已变异实测有牙。完整裁决与勘误:`docs/typing-policy-chokepoint-adr.md` 补记「源码忠实序号与 outdent 改号」。注意测试拼写陷阱:`1. 甲\n   2. 乙` **不是**嵌套形状(`   2.` 不能打断段落,是惰性续行)——"编号已正确"钉必须用 `1. 甲\n   1. 丙\n   2. 乙`。钉:`test-source-kernel-indent.mjs` OUTDENT ORDERED-MARKER RENUMBER 节(修复轮真实键盘序列现产出与预期逐字节一致的 `3. out`);通道仿真铁律:CDP `rawKeyDown` 不生成字符(内核 keymap 键照常、字符键会整键丢失),仿真真实键盘必须 `type:'keyDown'` 带 `text`,且标点 virtualKeyCode 不能用 `charCodeAt`(`.` 会算成 46=VK_DELETE 被当 Delete 处理)。

**2026-08-23(第八批)mark 输入规则接通内核——手打 `**粗**` 不再吞键(用户指令「开工」,承接第七批探索会话的 9-veto 家族)**。机制:markRule(`@milkdown/prose`)的事务**删开定界符+加 mark、从不插入所敲字符**,且 `run()` 给每个 undoable 规则的事务盖 `customInputRulesKey` meta `{transform, from, to, text}`——所敲字符和位置就在 meta 里。内核的回答是源码权威的极简形:**所敲定界符按字面字节插入 raw 光标处**(定界符本来就是 mark 的 markdown 拼写),走既有 plain-text 管线的全部守卫(gateway 重构出 `commitPlainTextSteps` 共用核,`commitMarkInputRule` 合成单步);网关新分类 `mark-input-rule`(meta 前缀 `MILKDOWN_CUSTOM_INPUTRULES` 扫描——纯数据读取不破坏 gateway 的零 @milkdown import 约束;两道结构闸:`from===to`+非空 text、**块骨架不变**〔node 规则天然不过〕)。控制器双路:**重解析证明** `safeParse(候选字节).eq(事务结果 doc)` 成立→字节发布+事务放行(shape (b));不成立→**字面回退**(`mark-input-rule-literal-fallback` 诊断):字节照落、规则事务否决、视图经 `applyKernelTransaction` 从解析重建——**击键永远落地,字节永远正确**。字面回退正是 `~~删~~` 可打的原因:milkdown 贪婪 `~{1,2}` 删除线规则在**第一个**闭合 `~` 上过早开火且拼写 GFM 读不回(`~~删~` 系 run 不配对的字面),回退落下该 `~`,第二个 `~` 经解析天然成删除线。行内代码是**本 app 自建**补全(editor-inline-code.js 闭合分支),不带 milkdown meta——现在盖 app 自己的 `horsemd-mark-input-rule` 同款 meta,一个分类吃两个来源。`**粗**` 全链验证:第一个闭合 `*` 无规则命中走 plain-text,字节 `**粗*` 重解析为 `*`+em(粗)(CommonMark 官方读法)、修复链收敛,第二个 `*` 的 strong 规则重解析证明通过——中间态就是正确性的一部分。**测试 fixture 陷阱**:`**粗*` 手造字面 PM 文档配不上内核 mdast(em 读法)→ 该块只读,无头钉必须用两侧皆字面的形状(未闭合反引号)。钉:mode-headless Case IR1-IR3(em 放行/行内代码放行/字面回退)、**新 `test:kernel-mark-inputrule-ui`**(LF+CRLF:真实键盘管线逐键打全五 mark 串,DOM 五 mark 渲染断言+字节逐字相等+保存冷重开,已入 `test:kernel-ui`)。真实 app 复验:第七批的对抗串 `*斜*与**粗**与\`码\`与~~删~~与==高==完` 现**逐字节精确落盘**,仅 1 次具名 literal-fallback(设计内)。新 KERNEL_CODES:`MARK_INPUT_RULE`(mark-input-rule-unprovable,i18n zh/en)——当前仅在 commit 失败路径使用,重解析失配已由字面回退接管。

**2026-08-24「21.md 无法保存」= 被拒击键的假脏(pendingRichEdit)**。用户报告 21.md 显示已修改+保存无效;实测**保存本身始终成功**(字节写盘、FAB 清除),真正的缺陷是:`1. 2.3121312 / 2. 2131 / 3. 4.` 的两个 authored 空项配对只读,往里打字被拒(字节零变化、具名 toast)——但 DOM `input` 事件仍无条件触发 `markRichEditPending`,`pendingRichEdit=true` 点亮 已修改+保存FAB,~500ms 后 rich-dirty reconcile 提交内核快照才自清;用户持续在只读段落尝试打字 → 每键重新点亮 → 呈现为「永久已修改、怎么保存都没用」。**修复**:Editor.jsx 的 `onRichEditPending` 包装在内核未降级时跳过该提示(call-time 判定 `kernelController && !isDegraded()`;提示存在的唯一理由是 legacy 200ms 序列化防抖,内核 onChange 同步发布,提示只会说谎)——degraded tab 回 legacy 照常拿提示,`test:rich-dirty-indicator-ui` 的 31ms 即时反馈钉不受影响。钉:**新 `test:kernel-false-dirty-ui`**(已入 `test:kernel-ui`;LF-only,家族无行尾面)。**复现考古教训(写给下一个追竞态的人)**:该假脏是**亚秒瞬态**——断言晚 500ms 就永远绿;确定性复现 = IME 尝试(回滚)后紧跟真实 keyDown 击键、**立即**读 FAB;插入 sleep 或把探针读延后都会漏。**残余已知缺口**:`3. 4.` 同行嵌套形状的两个空项本身仍只读(authored 空项不在 seed 机制覆盖内,typing 拒绝是记录在案的 fail-closed;绕行=源码模式直接编辑该两行)——列为下一写路径目标。**→ 已于同日收口(下段)。**

**2026-08-24(续)同行嵌套裸项的 Backspace 删除落地(用户追报「删除键删除报错」,问「是因为富文本的保护吗」——是:mdBlock-null 对的 fail-closed)**。三层修复:(1) **纯命令** `liftEmptyListItem`(commands/delete.js)新增 SAME-LINE NESTED EMPTY ITEM 分支:嵌套空项坐在**父项自己的 marker 行**且 `end===parent.end` 时,把嵌套 marker 字节改写为账本化种子 NBSP(`3. 4.` → `3.  `——indent 侧 EMPTY-ITEM SEED RESCUE 的既有拼写,配对可证、可打字、种子在首字符下溶解),第二下 Backspace 走既有视觉空项退出(清 marker 行留空行,既有拼写);重解析证明 `provenSeededSameLineDelete`(listItem 计数恰 -1、heading 不变、叶子多重集恰 +NBSP、种子段落恰在改写 offset 且有 listItem 祖先);带嵌套兄弟的形状照旧拒绝(改写会孤儿化兄弟)。**offset 解析陷阱**:空项光标在 `contentStart===end`,一切 end-exclusive span 都不含它——`listItemAt(offset) ?? listItemAt(offset-1)` 回退,空性守卫防内容项借道。(2) **控制器路由**(editor-kernel-mode.js structuralHandler):`pmPosToRaw` 对该光标为 null(修复前 Backspace 在此以 UNMAPPED→blockReadOnly toast 拒绝,而**打字却能落**——它走的是 bare-marker 打字机 `handleMarkerFollowingText` 自己的 marker 通道,不经 map 解析器)——Backspace/Delete 现回退 `markerRawOffsetAt`(打字通道同一个推导:mdItem.contentStart),**只放宽路由不放宽写入**(router 的整项命令自证空性)。(3) 钉:纯套件同行嵌套节(LF+CRLF+二连 Backspace 链+offset 26 router 用例+兄弟负控)、**新 `test:kernel-nested-bare-backspace-ui`**(LF+CRLF:真实 app 两下 Backspace 删净两个裸项+续打成段+字节精确落盘+冷重开无残留,已入 `test:kernel-ui`)。诊断线索留档:diag 环的 `block-read-only` 条目即可锁定 structuralHandler 的 UNMAPPED 分支(notifyRefusal 经 degradedPairAt 显示 blockReadOnly 文案,别被 toast 字面误导成 charMap 家族);`ol > li` 选择器在 milkdown 的列表 DOM 不匹配(li 非 ol 直系子),UI 探针勿用。

**2026-08-24(再续)MARKER-ESCAPING DELIMITER——`3. 4.` 的源头治理(用户报告「从 2 的序列转到 3,然后 3,4 一起出来了」)**。实测源头:Enter 出 `3.` 正常,用户打 `4` 落为文本,打 `.` 的瞬间字节 `3. 4.` 被 CommonMark 读成嵌套空项 → 视图当场裂开。裸 marker 终态的固有歧义(`4.` 停手=字面文本还是待补空格的 marker)按 Typora 答案落地:**会重构的 `.`/`)` 以转义拼写提交**(`4\.`——标准 CommonMark、所有渲染器显示 "4.",先例=autolink 的 `www\.`),**空格补全学会识别转义 marker**(`4\.`+Space → 解开转义写 `4. ` 真嵌套 marker——type-to-create-list 完整保留,`4` `.` `Space` `x` 实测得 `3. 4. x` 真嵌套)。三件套:纯命令 `spellMarkerEscapingDelimiter`(marker-space.js:数字前缀预筛+**双重解析证明**——未转义候选必须真重构、转义候选必须骨架全同且恰一个叶子从 `L` 变 `L+char`;小数 `4.5`、行中 `4.`、verbatim 全部 not-structural 走普通文本路);`spellMarkerCompletingSpace` 的 escaped-marker 臂(`(\d{1,9})\\[.)]$` → 解开+补全,同一 provenMarker 证明);控制器 `handleMarkerEscapingDelimiter`(markerInputPlugin 链中 growth 之后 following-text 之前,同 fail-closed try/catch 纪律)。**钉翻转(有据,三处)**:gesture-loop(burst 字节 `1. 2\.312`、`3. 4\.`,骨架不再含同行嵌套 `ol[li:]`)、marker-text(§2/存盘 `1\.x`)、render-consistency(burst `2\.9876543`);渲染文本全部不变,Tab 嵌套/marker-following/空格补全家族照旧。marker-space 纯套件新增转义两节(LF+CRLF+四负控)。注意:同行嵌套裸项从此**打字不可再产生**(只能 authored/粘贴进入),其 Backspace 删除路径(上段)与渲染忠实投影不变。

**2026-08-24(三续)转义的 IME 通道 + 「还是一样的」两案定性**。用户复测「还是一样」,两个成因:(1) **IME 组合提交绕过转义**——转义最初只挂 markerInputPlugin 的 handleTextInput,composition 提交(中文输入法的常态路径)不经过它,组合提交的 `4.` 照旧写字面字节裂项。修复=commitReplace 里的第三次「同命令、此路径也要」(与 block-tail heal、task-seed dissolve 同族同位):对以 `.`/`)` 结尾的 healInsert,构造 head-text(除末字符外全部就位)后咨询同一个 `spellMarkerEscapingDelimiter`,ok 则把末字符替换为转义拼写;not-structural 照旧字面提交。钉:**新 `test:kernel-ime-marker-escape-ui`**(LF+CRLF:真实 composition 提交 `4.`→字节 `3. 4\.`、视图单项、保存逐字节,已入 `test:kernel-ui`)。(2') **2026-08-24 方法论转向(用户评审「填鸭式修复」成立)**:见
**`docs/typing-policy-chokepoint-adr.md`**——裁决=打字拼写策略收敛到
`commitPlainTextSteps` 单一咽喉(IME commitReplace 路由进它,heal/种子溶解/
转义三份通道副本删除),拼写判定改为消费 vendored
`mdast-util-to-markdown/lib/unsafe.js`(remark 序列化器自身的权威转义表,
手写数字预筛只覆盖其十三条中的两条)。两台仪器已先行落地:
**`test:kernel-typing-policy-matrix`**(无头,入 kernel-headless:unsafe 全
字母×底座×块首尾扫描,restructures 快照化,当前洞图 24 条两族——`>` 任意
块首 18 条〔提前捕获的下一个洞〕+ 数字后 `.`/`)` 4 条〔原语层如实暴露,
咽喉统一后归零〕;加宽即失败)与
**`test:kernel-channel-equivalence-ui`**(入 kernel-ui:同组编辑经
键盘/insertText/IME 三通道存盘字节必须逐字相同——通道盲区家族的结构性钉)。
重构执行时以这两台仪器为网,目标态=typing-policy 快照清零。
**→ 第一阶段已执行(同日)**:`commands/text-escape.js` `escapePolicyForInsert`
= unsafe 表 gate + 对照候选双重解析证明(参照系=惰性 'x' 候选骨架;块级白名单
签名——行内 emphasis 不算重构);**TRANSIENT_SINGLE 例外**(`- + * > # \` ~ _`
单字符=裁决内裸 marker 中间态,永不转义——缺它时 `\*斜*` 级联毁 mark 线,
IR1/三 UI 套件抓获;sweep 单列 transients 钉 20 条);三调用点
(replaceVisibleText / commitPlainTextSteps / IME commitReplace)同一函数,
`spellMarkerEscapingDelimiter`+markerInputPlugin escape handler 删除;快照
...
text,诚实双号)——收编的键是**空格**。
**→ 同日终局(用户源码贴出 `2. 3\.`/`3. 4\.`,续行键是回车)**:splitListItem
新增 **NUMBER-ADOPTING ENTER** 分支(enter.js):项的全部内容恰为转义手打编号
`N\.` 且光标在项尾时,回车=收编 N 为本项 marker + 续 `N+1. ` 新项
(`2. 3\.`+回车 → `3. \n4. `,重解析证明两项各携期望编号、叶子恰失 `N.`);
中段回车/尾随内容照旧普通分裂。实测用户键序 `回车→3.→回车→4.→回车` 收敛为
`3. \n4. \n5. \n` 单号链。钉:commands 纯套件 NUMBER-ADOPTING 节
(LF+CRLF+`)` 族+两负控)、manual-number-ui 扩 Enter 流。

**2026-08-24(架构轮)三方评审 + 第二阶段落地**。用户问「是否架构出了问题」,
三个并行子代理评审(模型优先系逐行考据〔Milkdown ordered_list 带 order 属性,
wrapInOrderedListInputRule 以空格为提交手势收编数字——与我们同构〕、源码优先
系考据〔CM6/Obsidian/Vditor/Typora:字节权威不是问题,缝在「对光标块进行中
字节做全强度结构投影」,成熟系一律由下一击键/settle 裁决终局读法〕、本仓库
架构评审)一致裁决:**架构成立不换**——缺陷族两个发生器:①策略住通道上
(真架构错,已由咽喉统一+双仪器结构性关闭)②字节权威+即时投影固有缝
(补偿=转义↔收编对偶,即现行设计)。模型优先迁移与光标字面区均逐条否决,
详见 ADR 第二阶段裁决节。落地三件:(1) **未决态自动序号灰显**(纯装饰:
editor-kernel-undecided-ordinal.js 的 Decoration.node,类落在 Crepe list-item
node view 外层 `.milkdown-list-item-block`〔PM 对自定义 node view 的 node
decoration 应用在外层 dom——测试/CSS 都按此层级〕;CSS 灰 `.label.ordered`
自动号,绝不灰用户打的号;仅 kernel 模式挂载;钉
`test:kernel-undecided-ordinal-ui` 入 kernel-ui)。(2) **未决态×下一键演绎
完备表** `test:kernel-undecided-edges`(3 底座×LF/CRLF×7 出边=42 行,纯命令
逐格执行+快照双向钉,入 kernel-headless 链尾)——它当场抓出真缺陷:
**收编用了所打定界符,`4)` 在 `.` 列表裂成两个列表**(CommonMark 异定界符=
异列表);修复=**定界符跟随列表**(收编取编号、定界符用 item 自己的,
marker-space RENUMBER 臂与 enter NUMBER-ADOPTING 同步),两纯套件 `)` 用例
按新裁决翻转,42 行快照重基线。(3) ADR 增第二阶段裁决节(含 TRANSIENT_SINGLE
与数字 marker 默认不对称的显式条款)。**剩余(下会话)**:IME commitReplace
的 step 级路由收编——三项策略逻辑本体已全共享(咨询块非副本),通道等价钉
在岗,风险受控。出边表另记两条待裁决:U3 E-tab 的 `1.  ` 双空格拼写疣;
U2/U1 表已成为该域的权威行为文档。

**2026-08-24(五续)源码忠实序号显示(用户报告「这个源码都不对照」:源码
`1./3./4.` 视图显示 1/2/3)**。CommonMark 渲染惯例是序号按序列位置连续,但
字节权威编辑器(Obsidian/VSCode 同读法)应显示**作者自己的编号**。落地=
kernel-mode 内 `sourceOrdinalPlugin`(纯装饰):遍历投影图 list_item 对,
按 pair.mdBlock 的 mdast 起点从语法索引取 `item.ordered.number`(mdast 丢
每项编号,索引保留),Decoration.node 在 node view 外层 wrapper 设
`hm-source-ordinal` 类 + `--hm-source-ordinal` CSS 变量,app.css 用
::before content 换显(原 label 文本 visibility:hidden 保留——textContent
不变,既有按 textContent 断言的套件全部无扰)。陈旧图守卫:每对从当前 doc
重解析 nodeAt 校验,失配即跳过(装饰可暂缺、永不错)。legacy tab 零影响
(对照组钉)。IME 提交同日完成 step 级路由收编(commitResolvedTextSteps,
ADR 两阶段全部执行完毕)。钉:`test:kernel-source-ordinal-ui`
(kernel 三项 1./3./4. 逐项断言 + legacy 对照 + 字节不动,已入 kernel-ui)。
...
用户报告：引用里删掉/回车空任务项后「一下就被甩出引用区域」。实测三个缺陷叠加：占位光标落在 `> ` 行行尾边界时 `listItemAt` 仍把它算进邻近列表（mdast span 触边），Enter 被路由成 split-list-item、凭空造出 `> - [ ]  `、证明失败后回滚 + projection-mismatch，且视图光标已被甩到引用外（字节还留着 `> ` 行——字节/视图分裂）。修复后是 Typora/飞书式**分级退出**，每次 Enter 只走一级：嵌套空项 → 先降一级（`exitEmptyListItem` 对 depth>0 委托 `outdentListItem`，降不动才退回整行删除）；顶层空项 → 引用内空行（占位在引用内，可继续写引用内容）；空引用行 → 退出引用（`exitEmptyQuoteLine` 删掉 `> ` 行；旧的 `listItemAt` 前置守卫已删——纯引用行正则本身更强，且该守卫正是误拒来源；router 的 Enter 分支加了同款 quote-only-line 守卫）。退出后的占位新增 `outsideQuote` 落点（`placeholderForUnmappableAnchor`）：放到引用节点**之后**而非引用内容尾（否则打字会写成 `> 正文` 重新入引）；实测打字得 `...1231312\n\n新\n\n尾段`，无 lazy continuation。钉子：`test:kernel-quote-staged-exit-ui`（四段阶梯逐级断言光标归属 + 零错误 toast + 最终字节）；quote-enter-table / list-exit-placeholder / indent-empty-item / structural-matrix / position-matrix / quote-delete / task-item / gesture-loop 复跑全绿。**2026-08-31 补充裁决（用户「改成不退」）**：**整个引用只有一个空行**（刚建好还没写内容——`> `、裸 `>`，文档中段或末尾）时，Enter 是 SILENT_NO_OP——既不删引用也不落到 splitTextBlock（router 显式透传 silent-no-op，extendTrailingPlaceholder 同样吞键不 toast）；Backspace 仍是键盘删除空引用的出口。引用**尚有内容行**时空行 Enter 的分级退出保持不变（键盘离开引用的唯一路径）。exitEmptyQuoteLine 用「候选删除会使 blockquote 计数减一」判定整引用为空。quote-enter-table 1c 案例同日翻转（原钉「一个回车引用消失」正是被推翻的行为）；无头钉子在 test-source-kernel-commands.mjs 的 QUOTE-LINE ENTER ROUTING 节。

### 5.2i 引用上下文补全（2026-08-31，用户「引用为啥无法使用分隔符」+「审查引用在内核中被关闭的原因」）

审查结论：引用链上仍被关的斜杠项只剩 `/divider`（引用末尾无后续行时具名拒绝 no-caret-home）、`/image`、`/text` 三个，全部补通，共用一套机制——**光标的家直接写进字节**：引用末尾插入时命令追加一个空引用续行（`> ---` + 行尾 + `> `，quoteSpelling 同款前缀，嵌套 `> > ` 同理），routed 结果带 `quotePlaceholder` 标志；控制器先 `applyKernelTransaction`，锚可解析（裸引用行虚拟对）就 `setCaretFromRaw` 直落，否则 `placeholderForUnmappableAnchor` 物化引用内占位（其后向字节探测在「引用里只有 hr 等无 charMap 原子」时找不到任何可映射位——新增 blockPairs 兜底：取锚前最近 block pair 的 pmPos 定位引用祖先）。打字提交 `> 正文` 续行（实测 `> ---\n> \n> 正文\n`）。`/image` 解除引用门禁后走同路（`> ![]()\n> `，图卡在引用内渲染）。`/text` 新增 `revertQuotedText`：删查询块内容字节、留 `> ` 空引用行（一等分隔符，无合并风险），锚即行前缀后；其证明**刻意不用**共享的容器重叠签名——删除会让 blockquote 报告的 span 缩短（remark 不再把尾部空引用行计入），overlap 判定会单边签名容器（实测引用末尾拒绝、引用中段通过的不对称）——改为两侧都无条件下钻 blockquote、非容器节点全签名（候选偏移回移）、基线只跳过查询块本身、外加 blockquote 计数不变断言。axis(a) `provenSpanEnd` 增加镜像放宽：插入节点在 insertedEnd 之前结束时，只允许差额恰为本命令自己写的 quoteTail 且逐字节为引用前缀/空白。仍拒绝：引用内**列表**链上的 /text、/table 等（list-ancestor 通用拒绝）。钉子翻转/新增均在 test-source-kernel-blockinsert.mjs 原地（quote-END divider、quoted /image 两形、quoted /text 四形）；UI 复跑 blockinsert / slash-combo / leaf-insert / quote-staged-exit 全绿，真实应用逐项探针含打字后字节。

### 5.2j 引用内占位会话废弃时的空引用行回收（2026-08-31 记录，同日实现）

用户观察：在引用里回车得到空引用行占位后，若不是再按回车退出、而是**点到引用外**直接写字，会话写入的 `>` 空行字节留在源码里（富文本按 CommonMark 不渲染空引用行，两边「看起来」不一致；语义一致、收尾正确）。legacy 不会累积——它每次序列化都丢空引用行；内核字节权威、不做静默正规化，但「会话自己写的、从未填充的空引用行」按票据学说应在会话废弃时回收。实现（同日，用户「立即修复」）：`reclaimAbandonedQuoteLines`（editor-kernel-mode.js），挂在两个结束会话的主通路尾部——applyKernelTransaction 的成功尾（trailing-atom 等结构提交）与 plain-text case。仅回收 voucher 记录的 `writtenFrom/To` span（会话自己写的字节，绝不动作者字节）；结束提交若真重叠 span 则跳过；「填充」判别 = 在 span 末端插入且**不以换行开头**（尾部虚拟段落的 `\n\n…` 分隔提交与 span 末端重合，实测曾整体挡住回收）；span 经结束提交 delta 校正后逐字节复验仍为纯空引用行（正则要求每行含 `>`——顶层空行 run 永不匹配，gap-enter 会话不受影响）；光标锚三级兜底（map 解析 → ending 事务 selection → 调用点显式传入，orphan 修复期 map 为 null 是常态）；回收是**记入历史**的普通内核事务（不记会腐蚀 source history 的逆放）。诊断：`quote-line-reclaimed`；调试细节 `globalThis.__hmReclaimDebug`。钉子：quote-enter-table 1e（回车开会话→点走打字→存盘字节零残留）。
