...
> **编者按（2026-08-18，事后更正，不改动本设计文档的历史记录）**：本段“显示为禁用”这一具体交互决定**未被采纳**——实现改为按钮照常可点，点击后走真实的 fail-closed veto + toast（“暂不支持”或“此段落只读”，取决于具体原因），而不是提前置灰。更重要的是，加粗/斜体/链接三者本身早已不再属于“尚未迁移”：计划四（行内 marks + 引用域）之后三者均已支持施加/撤销/改写。本文件是设计阶段的原始记录，保留供历史追溯；**当前真实覆盖面与阻止矩阵以 `docs/ai-handoff.md` §5.2d–§5.2g 为准**，请勿据本段字面意思判断某项操作现在是否可用。
