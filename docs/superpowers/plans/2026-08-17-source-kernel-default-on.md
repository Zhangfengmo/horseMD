...
**首选方案 (d)：让内核镜像同一套分块。** 内核用**同样的 `splitMarkdown` 边界**逐块解析，再把每块 mdast 的 position 按该块的原文起始偏移平移后拼接。这样两侧的块序列**在构造上就一致**，正是本架构"两链一致"原则的直接应用。可行性已用探针在真实多分块文档（7 块 LF / 8 块 CRLF）上验证：
