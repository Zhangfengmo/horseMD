...
async function run() {
  ...
  try {
    ...
    // =====================================================================
    // 5b) Delete AND Backspace at a heading's CONTENT START: the demote
    //     GESTURE (2026-08-21 it was a named refusal; 2026-08-22 the kernel
    //     writes the marker change itself — commands/block-type.js
    //     `demoteHeadingAtCaret`, full matrix in
    //     test-kernel-heading-demote-ui.mjs). Here the H1's content starts
    //     with session-written no-break spaces, so what this file pins is the
    //     INTERSECTION with its own domain: the demote deletes exactly the
    //     `# ` opening, the ledgered U+00A0 run survives byte for byte, and
    //     kernel undo restores the heading — for BOTH keys.
    // =====================================================================
    {
      const before = await readSource(evaluate, 'before the heading-demote probe')
      const demoted = before.replace('# ' + NBSP + NBSP + NB2, NBSP + NBSP + NB2)
      assert.notEqual(demoted, before, 'the probe fixture must contain the NBSP-led H1')
      for (const key of ['Delete', 'Backspace']) {
        await clickAt(evaluate, send, NBSP + NBSP + NB2 + '标题甲', 0)
        await pressKey(send, { key, code: key })
        await sleep(450)
        assert.equal(await readSource(evaluate, `after ${key} at the heading start`), demoted,
          `${key} at the H1's content start must demote it to a paragraph, deleting exactly the ATX opening`)
        // Kernel undo (Mod-z; Meta on darwin = modifiers 4) restores the
        // heading — the demote holds a history slot like any structural edit.
        await pressKey(send, { key: 'z', code: 'KeyZ', modifiers: 4, windowsVirtualKeyCode: 90, nativeVirtualKeyCode: 90 })
        await sleep(450)
        assert.equal(await readSource(evaluate, `undo after ${key}`), before,
          `undo must restore the H1 the ${key} demote removed`)
      }
