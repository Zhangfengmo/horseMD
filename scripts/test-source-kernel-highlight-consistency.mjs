...
//   D. Declared DIVERGENCES — the two shapes where the rules genuinely cannot
//      agree (markers written escaped or as character references) are pinned
//      as kernel-side FAIL-CLOSED: zero kernel highlights, block degrades,
//      never a guess.
...
const CORPUS = [
  ...
  // --- declared divergences (kernel fails closed) --------------------------
  ['\\=\\=x\\=\\=', 'escaped markers', { divergent: true }],
  ['&#61;&#61;x&#61;&#61;', 'markers written as character references', { divergent: true }]
]
...
for (const [text, label, options = {}] of CORPUS) {
  ...
  for (let i = 0; i < editorBlocks.length; i += 1) {
    ...
    if (!charMap) {
      // A block the kernel cannot character-map at all, for reasons that have
      // nothing to do with `==` (today: gfm autolink-literal `link` nodes,
      // which remark produces WITHOUT a position). It degrades to read-only —
      // fail-closed, pre-existing, and pinned here so it cannot silently
      // become a mis-map.
...
// assert A + C on each. Catches shapes nobody thought to enumerate.
// ---------------------------------------------------------------------------
{
  const pieces = ['==', '=', 'a', ' ', '高', 'x', '{', '}', '*', '`', '\\', '&amp;', '.', '\n\n', '_']
  let seed = 20260817
...
console.log(
  ...
  `${divergences} declared divergences fail closed, 4000 randomized documents)`
)
